...
```python
user_client.shared_links_web_links.find_web_link_for_shared_link(''.join(['shared_link=', web_link_from_api.shared_link.url, '&shared_link_password=Secret123@']))
```
...
```python
client.shared_links_web_links.update_shared_link_on_web_link(web_link_id, 'shared_link', shared_link=UpdateSharedLinkOnWebLinkSharedLink(access=UpdateSharedLinkOnWebLinkSharedLinkAccessField.COLLABORATORS))
```
