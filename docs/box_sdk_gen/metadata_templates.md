...
```python
client.metadata_templates.get_metadata_template(GetMetadataTemplateScope.ENTERPRISE, template.template_key)
```
...
```python
client.metadata_templates.update_metadata_template(UpdateMetadataTemplateScope.ENTERPRISE, template_key, [UpdateMetadataTemplateRequestBody(op=UpdateMetadataTemplateRequestBodyOpField.ADDFIELD, field_key='newfieldname', data={'type': 'string', 'displayName': 'newFieldName'})])
```
