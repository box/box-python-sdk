...
```python
client.search.search_by_metadata_query(search_from, '0', query='name = :name AND age < :age AND birthDate >= :birthDate AND countryCode = :countryCode AND sports = :sports', query_params={'name': 'John', 'age': 50, 'birthDate': '2001-01-01T02:20:10.120Z', 'countryCode': 'US', 'sports': ['basketball', 'tennis']})
```
...
```python
client.search.search_for_content(ancestor_folder_ids=['0'], mdfilters=[MetadataFilter(filters=search_filters, scope=MetadataFilterScopeField.ENTERPRISE, template_key=template_key)])
```
