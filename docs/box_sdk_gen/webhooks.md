...
```python
client.webhooks.create_webhook(CreateWebhookTarget(id=folder.id, type=CreateWebhookTargetTypeField.FOLDER), 'https://example.com/new-webhook', [CreateWebhookTriggers.FILE_UPLOADED])
```
...
```python
WebhooksManager.validate_message(body, headers_with_correct_datetime, primary_key, secondary_key=secondary_key)
```
