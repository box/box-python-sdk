...
This document focuses on helping you migrate code from the manually maintained `boxsdk` module to the generated `box_sdk_gen` package.
Many APIs were redesigned for consistency and modern Python patterns, so this guide calls out how to adopt the new shapes safely and incrementally.
...
We recommend using `box_sdk_gen` as the preferred SDK going forward. This SDK is automatically generated from the
...
Firstly, in the `boxsdk` package, to be able to perform any action on an API object, e.g. `User`, you first had to create its class.
...
In the `box_sdk_gen` package, the API methods are grouped into dedicated manager classes, e.g. `User` object
...
e.g. to update a user's name, call method:
...
In `boxsdk` package, there were no data types explicitly defined -
...
In `box_sdk_gen` package, the data classes are defined in `schemas` module, so you know, which fields to expect before
...
The new `box_sdk_gen` package is designed to be mostly immutable. This means that methods,
which used to modify the existing object in `boxsdk` package now return a new instance of the class with the modified state.
...
The `box_sdk_gen` package offers the same authentication methods as `boxsdk` package.
...
The `box_sdk_gen` package, provides a convenient `BoxDeveloperTokenAuth`, which allows authenticating
...
In `boxsdk` package, method for user authentication was named `authenticate_user(self, user: Union[str, 'User'] = None) -> str`
...
In new `box_sdk_gen` package, to authenticate as user you need to call
...
In `boxsdk` package `CCGAuth` was accepting both user object and User ID. In `box_sdk_gen` package
the `BoxCCGAuth` constructor accepts only User ID instead.
...
In `boxsdk` package, there were two methods which allowed to switch between using service and user account. Calling these methods
...
In the `box_sdk_gen` package, to keep the immutability design, the methods switching authenticated subject were replaced with methods
...
To get authorization url in the `box_sdk_gen` package, you need to first create the `BoxOAuth` class (previously `OAuth2`) using
...
In `boxsdk` package you could provide a `store_tokens` callback method to an authentication class, which was called each time
...
What is more, `boxsdk` package allowed also to provide `retrieve_tokens` callback, which is called each time the SDK needs to use
...
In the `box_sdk_gen` package, you can define your own class delegated for storing and retrieving a token. It has to inherit from
...
The process of downscoping token in the new package is similar to the old one. The main difference is that the new method
...
To revoke current client's tokens in the `box_sdk_gen` package, you need to call `revoke_token` method of the auth class instead of
`revoke` method.
...
In `box_sdk_gen` package the method was renamed to `with_as_user_header(self, user_id: str) -> BoxClient`
...
In manual `boxsdk` package, you could specify the custom base URLs, which will be used for API calls made by setting
...
In the new package this functionality has been implemented as part of the `BoxClient` class.
...
In the new `box_sdk_gen` package, the `WebhooksManager.validate_message()` method requires the `body` to be of type `string` and
...
In `boxsdk`, you could use the `get_chunked_uploader()` method to create a chunked uploader object.
...
In `box_sdk_gen`, the equivalent method is `chunked_uploads.upload_big_file()`. It accepts a file-like object
