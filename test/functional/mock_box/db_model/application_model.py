...
_user_app_table = sqlalchemy.Table(
    ...
    DbModel.metadata,
    sqlalchemy.Column(
        'app_id', sqlalchemy.Integer, sqlalchemy.ForeignKey('box_application.id')
    ),
    sqlalchemy.Column(
        'user_id', sqlalchemy.Integer, sqlalchemy.ForeignKey('box_user.id')
    ),
)
...
class ApplicationModel(DbModel):
    ...
    auth_tokens = relationship(
        'TokenModel', backref='application', cascade='save-update, delete'
    )
