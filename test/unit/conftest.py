...
from boxsdk.config import API, Client, NetworkProxy
...
def mock_box_session(translator):
    ...
    mock_session._network_proxy_config = mock_session.network_proxy_config = NetworkProxy()
...
def mock_box_session_2(translator):
    ...
    mock_session._network_proxy_config = NetworkProxy()
    # pylint:enable=protected-access
    mock_session.get_url.side_effect = lambda *args, **kwargs: Session.get_url(mock_session, *args, **kwargs)
    mock_session.translator = translator
    return mock_session
