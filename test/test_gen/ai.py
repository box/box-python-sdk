...
from box_sdk_gen.schemas.ai_extract_structured_response import AiExtractStructuredResponse
...
from box_sdk_gen.managers.metadata_templates import CreateMetadataTemplateFieldsTypeField

from box_sdk_gen.managers.metadata_templates import CreateMetadataTemplateFieldsOptionsField
...
def testAskAISingleItem():
    ai_ask_agent_config: AiAgentAsk = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.ASK, language='en-US')
    file_to_ask: FileFull = upload_new_file()
    response: Optional[AiResponseFull] = client.ai.create_ai_ask(CreateAiAskMode.SINGLE_ITEM_QA, 'which direction sun rises', [AiItemAsk(id=file_to_ask.id, type=AiItemAskTypeField.FILE, content='Sun rises in the East')], ai_agent=ai_ask_agent_config)
...
def testAITextGenWithDialogueHistory():
    file_to_ask: FileFull = upload_new_file()
    ai_text_gen_agent_config: AiAgentTextGen = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.TEXT_GEN, language='en-US')
...
def testGettingAIAskAgentConfig():
    ai_ask_config: Union[AiAgentAsk, AiAgentTextGen, AiAgentExtract, AiAgentExtractStructured] = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.ASK, language='en-US')
...
def testGettingAITextGenAgentConfig():
    ai_text_gen_config: Union[AiAgentAsk, AiAgentTextGen, AiAgentExtract, AiAgentExtractStructured] = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.TEXT_GEN, language='en-US')
...
def testAIExtract():
    ai_extract_agent_config: AiAgentExtract = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.EXTRACT, language='en-US')
    long_text_config_with_no_embeddings: AiAgentLongTextTool = AiAgentLongTextTool(system_message=ai_extract_agent_config.long_text.system_message, prompt_template=ai_extract_agent_config.long_text.prompt_template, model=ai_extract_agent_config.long_text.model, num_tokens_for_completion=ai_extract_agent_config.long_text.num_tokens_for_completion, llm_endpoint_params=ai_extract_agent_config.long_text.llm_endpoint_params)
    agent_ignoring_overriding_embeddings_model: AiAgentExtract = AiAgentExtract(basic_text=ai_extract_agent_config.basic_text, long_text=long_text_config_with_no_embeddings)
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=''.join([get_uuid(), '.txt']), parent=UploadFileAttributesParentField(id='0')), string_to_byte_stream('My name is John Doe. I live in San Francisco. I was born in 1990. I work at Box.'))
    file: FileFull = uploaded_files.entries[0]
    delay_in_seconds(5)
    response: AiResponse = client.ai.create_ai_extract('firstName, lastName, location, yearOfBirth, company', [AiItemBase(id=file.id)], ai_agent=agent_ignoring_overriding_embeddings_model)
    expected_response: str = '{"firstName": "John", "lastName": "Doe", "location": "San Francisco", "yearOfBirth": "1990", "company": "Box"}'
    assert response.answer == expected_response
    assert response.completion_reason == 'done'
    client.files.delete_file_by_id(file.id)

def testAIExtractStructuredWithFields():
    ai_extract_structured_agent_config: AiAgentExtractStructured = client.ai.get_ai_agent_default_config(GetAiAgentDefaultConfigMode.EXTRACT_STRUCTURED, language='en-US')
    long_text_config_with_no_embeddings: AiAgentLongTextTool = AiAgentLongTextTool(system_message=ai_extract_structured_agent_config.long_text.system_message, prompt_template=ai_extract_structured_agent_config.long_text.prompt_template, model=ai_extract_structured_agent_config.long_text.model, num_tokens_for_completion=ai_extract_structured_agent_config.long_text.num_tokens_for_completion, llm_endpoint_params=ai_extract_structured_agent_config.long_text.llm_endpoint_params)
    agent_ignoring_overriding_embeddings_model: AiAgentExtractStructured = AiAgentExtractStructured(basic_text=ai_extract_structured_agent_config.basic_text, long_text=long_text_config_with_no_embeddings)
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=''.join([get_uuid(), '.txt']), parent=UploadFileAttributesParentField(id='0')), string_to_byte_stream('My name is John Doe. I was born in 4th July 1990. I am 34 years old. My hobby is guitar.'))
    file: FileFull = uploaded_files.entries[0]
    delay_in_seconds(5)
    response: AiExtractStructuredResponse = client.ai.create_ai_extract_structured([AiItemBase(id=file.id)], fields=[CreateAiExtractStructuredFields(key='firstName', display_name='First name', description='Person first name', prompt='What is the your first name?', type='string'), CreateAiExtractStructuredFields(key='lastName', display_name='Last name', description='Person last name', prompt='What is the your last name?', type='string'), CreateAiExtractStructuredFields(key='dateOfBirth', display_name='Birth date', description='Person date of birth', prompt='What is the date of your birth?', type='date'), CreateAiExtractStructuredFields(key='age', display_name='Age', description='Person age', prompt='How old are you?', type='float'), CreateAiExtractStructuredFields(key='hobby', display_name='Hobby', description='Person hobby', prompt='What is your hobby?', type='multiSelect', options=[CreateAiExtractStructuredFieldsOptionsField(key='guitar'), CreateAiExtractStructuredFieldsOptionsField(key='books')])], ai_agent=agent_ignoring_overriding_embeddings_model)
    assert to_string(get_value_from_object_raw_data(response, 'answer.hobby')) == to_string(['guitar'])
    assert to_string(get_value_from_object_raw_data(response, 'answer.firstName')) == 'John'
    assert to_string(get_value_from_object_raw_data(response, 'answer.lastName')) == 'Doe'
    assert to_string(get_value_from_object_raw_data(response, 'answer.dateOfBirth')) == '1990-07-04'
...
def testAIExtractStructuredWithMetadataTemplate():
    ...
    template: MetadataTemplate = client.metadata_templates.create_metadata_template('enterprise', template_key, template_key=template_key, fields=[CreateMetadataTemplateFields(key='firstName', display_name='First name', description='Person first name', type=CreateMetadataTemplateFieldsTypeField.STRING), CreateMetadataTemplateFields(key='lastName', display_name='Last name', description='Person last name', type=CreateMetadataTemplateFieldsTypeField.STRING), CreateMetadataTemplateFields(key='dateOfBirth', display_name='Birth date', description='Person date of birth', type=CreateMetadataTemplateFieldsTypeField.DATE), CreateMetadataTemplateFields(key='age', display_name='Age', description='Person age', type=CreateMetadataTemplateFieldsTypeField.FLOAT), CreateMetadataTemplateFields(key='hobby', display_name='Hobby', description='Person hobby', type=CreateMetadataTemplateFieldsTypeField.MULTISELECT, options=[CreateMetadataTemplateFieldsOptionsField(key='guitar'), CreateMetadataTemplateFieldsOptionsField(key='books')])])
    response: AiExtractStructuredResponse = client.ai.create_ai_extract_structured([AiItemBase(id=file.id)], metadata_template=CreateAiExtractStructuredMetadataTemplate(template_key=template_key, scope='enterprise'))
    assert to_string(get_value_from_object_raw_data(response, 'answer.firstName')) == 'John'
    assert to_string(get_value_from_object_raw_data(response, 'answer.lastName')) == 'Doe'
    assert to_string(get_value_from_object_raw_data(response, 'answer.dateOfBirth')) == '1990-07-04T00:00:00Z'
    assert to_string(get_value_from_object_raw_data(response, 'answer.age')) == '34'
    assert to_string(get_value_from_object_raw_data(response, 'answer.hobby')) == to_string(['guitar'])
    assert response.completion_reason == 'done'
    client.metadata_templates.delete_metadata_template(DeleteMetadataTemplateScope.ENTERPRISE, template.template_key)
    client.files.delete_file_by_id(file.id)