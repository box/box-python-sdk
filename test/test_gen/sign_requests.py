...
from box_sdk_gen.schemas.sign_request_create_signer import SignRequestCreateSignerRoleField
...
def testCreateGetCancelAndListSignRequest():
    ...
    assert created_sign_request.signers[1].declined_redirect_url == 'https://www.box.com'
    ...
    assert date_to_string(created_sign_request.prefill_tags[0].date_value) == '2035-01-01'
...
def testCreateSignRequestWithSignerGroupId():
    ...
    file_to_sign: FileFull = upload_new_file()
    destination_folder: FolderFull = create_new_folder()
    created_sign_request: SignRequest = client.sign_requests.create_sign_request([SignRequestCreateSigner(email=signer_1_email, signer_group_id='user'), SignRequestCreateSigner(email=signer_2_email, signer_group_id='user')], source_files=[FileBase(id=file_to_sign.id)], parent_folder=FolderMini(id=destination_folder.id))
    assert len(created_sign_request.signers) == 3
    assert not created_sign_request.signers[1].signer_group_id == None
    assert created_sign_request.signers[1].signer_group_id == created_sign_request.signers[2].signer_group_id
    client.folders.delete_folder_by_id(destination_folder.id, recursive=True)
    client.files.delete_file_by_id(file_to_sign.id)