...
def testZipDownload():
    ...
    zip_stream: ByteStream = client.zip_downloads.download_zip([DownloadZipItems(id=file_1.id, type=DownloadZipItemsTypeField.FILE), DownloadZipItems(id=file_2.id, type=DownloadZipItemsTypeField.FILE), DownloadZipItems(id=folder_1.id, type=DownloadZipItemsTypeField.FOLDER)], download_file_name='zip')
    assert buffer_equals(read_byte_stream(zip_stream), generate_byte_buffer(10)) == False
...
def testManualZipDownloadAndCheckStatus():
    ...
    file_2: FileFull = upload_new_file()
    folder_1: FolderFull = create_new_folder()
    zip_download: ZipDownload = client.zip_downloads.create_zip_download([CreateZipDownloadItems(id=file_1.id, type=DownloadZipItemsTypeField.FILE), CreateZipDownloadItems(id=file_2.id, type=DownloadZipItemsTypeField.FILE), CreateZipDownloadItems(id=folder_1.id, type=DownloadZipItemsTypeField.FOLDER)], download_file_name='zip')
    ...
    zip_stream: ByteStream = client.zip_downloads.get_zip_download_content(zip_download.download_url)
    assert buffer_equals(read_byte_stream(zip_stream), generate_byte_buffer(10)) == False
    zip_download_status: ZipDownloadStatus = client.zip_downloads.get_zip_download_status(zip_download.status_url)
