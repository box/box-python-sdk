...
from box_sdk_gen.managers.shield_information_barrier_segment_members import CreateShieldInformationBarrierSegmentMemberShieldInformationBarrierSegment

from box_sdk_gen.managers.shield_information_barrier_segment_members import CreateShieldInformationBarrierSegmentMemberShieldInformationBarrierSegmentTypeField
...
def testShieldInformationBarrierSegmentMembers():
    ...
    segment: ShieldInformationBarrierSegment = client.shield_information_barrier_segments.create_shield_information_barrier_segment(ShieldInformationBarrierBase(id=barrier_id, type=ShieldInformationBarrierBaseTypeField.SHIELD_INFORMATION_BARRIER), segment_name)
    assert segment.name == segment_name
    segment_member: ShieldInformationBarrierSegmentMember = client.shield_information_barrier_segment_members.create_shield_information_barrier_segment_member(CreateShieldInformationBarrierSegmentMemberShieldInformationBarrierSegment(id=segment.id, type=CreateShieldInformationBarrierSegmentMemberShieldInformationBarrierSegmentTypeField.SHIELD_INFORMATION_BARRIER_SEGMENT), UserBase(id=get_env_var('USER_ID')))
    assert segment_member.user.id == get_env_var('USER_ID')
    assert segment_member.shield_information_barrier_segment.id == segment.id
    segment_members: ShieldInformationBarrierSegmentMembers = client.shield_information_barrier_segment_members.get_shield_information_barrier_segment_members(segment.id)
    assert len(segment_members.entries) > 0
    segment_member_get: ShieldInformationBarrierSegmentMember = client.shield_information_barrier_segment_members.get_shield_information_barrier_segment_member_by_id(segment_member.id)
