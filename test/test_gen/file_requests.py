...
def testGetCopyUpdateDeleteFileRequest():
    ...
    file_request: FileRequest = client.file_requests.get_file_request_by_id(file_request_id)
    assert file_request.id == file_request_id
    assert to_string(file_request.type) == 'file_request'
    copied_file_request: FileRequest = client.file_requests.create_file_request_copy(file_request_id, CreateFileRequestCopyFolder(id=file_request.folder.id, type=CreateFileRequestCopyFolderTypeField.FOLDER))
    ...
    updated_file_request: FileRequest = client.file_requests.update_file_request_by_id(copied_file_request.id, title='updated title', description='updated description')
