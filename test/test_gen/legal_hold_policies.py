...
def testCreateNotOngoingLegalHoldPolicy():
    ...
    legal_hold_policy: LegalHoldPolicy = client.legal_hold_policies.create_legal_hold_policy(legal_hold_policy_name, description=legal_hold_description, filter_started_at=filter_started_at, filter_ended_at=filter_ended_at, is_ongoing=False)
    assert legal_hold_policy.policy_name == legal_hold_policy_name
    assert legal_hold_policy.description == legal_hold_description
    assert date_time_to_string(legal_hold_policy.filter_started_at) == date_time_to_string(filter_started_at)
    assert date_time_to_string(legal_hold_policy.filter_ended_at) == date_time_to_string(filter_ended_at)
...
def testCreateUpdateGetDeleteLegalHoldPolicy():
    ...
    legal_hold_policy_by_id: LegalHoldPolicy = client.legal_hold_policies.get_legal_hold_policy_by_id(legal_hold_policy_id)
    assert legal_hold_policy_by_id.id == legal_hold_policy_id
    legal_hold_policies: LegalHoldPolicies = client.legal_hold_policies.get_legal_hold_policies()
    assert len(legal_hold_policies.entries) > 0
    updated_legal_hold_policy_name: str = get_uuid()
    updated_legal_hold_policy: LegalHoldPolicy = client.legal_hold_policies.update_legal_hold_policy_by_id(legal_hold_policy_id, policy_name=updated_legal_hold_policy_name)
