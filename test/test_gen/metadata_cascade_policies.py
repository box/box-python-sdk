...
from box_sdk_gen.managers.metadata_cascade_policies import CreateMetadataCascadePolicyScope

from box_sdk_gen.schemas.metadata_cascade_policies import MetadataCascadePolicies

from box_sdk_gen.managers.metadata_cascade_policies import ApplyMetadataCascadePolicyConflictResolution
...
def testMetadataCascadePolicies():
    ...
    client.metadata_templates.create_metadata_template('enterprise', template_key, template_key=template_key, fields=[CreateMetadataTemplateFields(type=CreateMetadataTemplateFieldsTypeField.STRING, key='testName', display_name='testName')])
    folder: FolderFull = create_new_folder()
    enterprise_id: str = get_env_var('ENTERPRISE_ID')
    cascade_policy: MetadataCascadePolicy = client.metadata_cascade_policies.create_metadata_cascade_policy(folder.id, CreateMetadataCascadePolicyScope.ENTERPRISE, template_key)
    ...
    policy_from_the_api: MetadataCascadePolicy = client.metadata_cascade_policies.get_metadata_cascade_policy_by_id(cascade_policy_id)
    assert cascade_policy_id == policy_from_the_api.id
    policies: MetadataCascadePolicies = client.metadata_cascade_policies.get_metadata_cascade_policies(folder.id)
    assert len(policies.entries) == 1
    with pytest.raises(Exception):
        client.metadata_cascade_policies.apply_metadata_cascade_policy(cascade_policy_id, ApplyMetadataCascadePolicyConflictResolution.OVERWRITE)
    client.folder_metadata.create_folder_metadata_by_id(folder.id, CreateFolderMetadataByIdScope.ENTERPRISE, template_key, {'testName': 'xyz'})
    client.metadata_cascade_policies.apply_metadata_cascade_policy(cascade_policy_id, ApplyMetadataCascadePolicyConflictResolution.OVERWRITE)
