...
def testUserCollaborations():
    ...
    user: UserFull = client.users.create_user(user_name, login=user_login, is_platform_access_only=True)
    folder: FolderFull = create_new_folder()
    collaboration: Collaboration = client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FOLDER, id=folder.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.USER, id=user.id), CreateCollaborationRole.EDITOR)
    assert to_string(collaboration.role) == 'editor'
    collaboration_id: str = collaboration.id
    collaboration_from_api: Collaboration = client.user_collaborations.get_collaboration_by_id(collaboration_id)
    ...
    updated_collaboration: Optional[Collaboration] = client.user_collaborations.update_collaboration_by_id(collaboration_id, UpdateCollaborationByIdRole.VIEWER)
...
def testConvertingUserCollaborationToOwnership():
    ...
    collaboration: Collaboration = client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FOLDER, id=folder.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.USER, id=user.id), CreateCollaborationRole.EDITOR)
    assert to_string(collaboration.role) == 'editor'
    owner_collaboration: Optional[Collaboration] = client.user_collaborations.update_collaboration_by_id(collaboration.id, UpdateCollaborationByIdRole.OWNER)
    assert owner_collaboration == None
    folder_collaborations: Collaborations = client.list_collaborations.get_folder_collaborations(folder.id)
...
def testExternalUserCollaborations():
    ...
    folder: FolderFull = create_new_folder()
    collaboration: Collaboration = client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FOLDER, id=folder.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.USER, login=user_login), CreateCollaborationRole.EDITOR)
    assert to_string(collaboration.role) == 'editor'
    collaboration_id: str = collaboration.id
    collaboration_from_api: Collaboration = client.user_collaborations.get_collaboration_by_id(collaboration_id)
    assert collaboration_id == collaboration_from_api.id
