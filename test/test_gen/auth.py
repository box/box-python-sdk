...
def test_jwt_auth_downscope():
    ...
    uploaded_files: Files = parent_client.uploads.upload_file(UploadFileAttributes(name=get_uuid(), parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(1024 * 1024))
    file: FileFull = uploaded_files.entries[0]
    resource_path: str = ''.join(['https://api.box.com/2.0/files/', file.id])
    downscoped_token: AccessToken = auth.downscope_token(['item_rename', 'item_preview'], resource=resource_path)
    assert not downscoped_token.access_token == None
    downscoped_client: BoxClient = BoxClient(auth=BoxDeveloperTokenAuth(token=downscoped_token.access_token))
...
def test_jwt_downscope_token_succeeds_if_no_token_available():
    ...
    downscoped_client: BoxClient = BoxClient(auth=BoxDeveloperTokenAuth(token=downscoped_token.access_token))
    with pytest.raises(Exception):
        downscoped_client.uploads.upload_file(UploadFileAttributes(name=get_uuid(), parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(1024 * 1024))
...
def test_jwt_auth_revoke():
    ...
    assert not token_from_storage_before_revoke.access_token == token_from_storage_after_revoke.access_token
...
def test_oauth_auth_authorizeUrl():
    ...
    assert auth_url == 'https://account.box.com/api/oauth2/authorize?client_id=OAUTH_CLIENT_ID&response_type=code' or auth_url == 'https://account.box.com/api/oauth2/authorize?response_type=code&client_id=OAUTH_CLIENT_ID'

def test_oauth_downscope_token_succeeds_if_no_token_available():
    config: OAuthConfig = OAuthConfig(client_id=get_env_var('CLIENT_ID'), client_secret=get_env_var('CLIENT_SECRET'))
    auth: BoxOAuth = BoxOAuth(config=config)
...
def test_ccg_auth_downscope():
    ccg_config: CCGConfig = CCGConfig(client_id=get_env_var('CLIENT_ID'), client_secret=get_env_var('CLIENT_SECRET'), user_id=get_env_var('USER_ID'))
    auth: BoxCCGAuth = BoxCCGAuth(config=ccg_config)
    parent_client: BoxClient = BoxClient(auth=auth)
    folder: FolderFull = parent_client.folders.create_folder(get_uuid(), CreateFolderParent(id='0'))
...
def test_developer_downscope_token_succeeds_if_no_token_available():
    developer_token_config: DeveloperTokenConfig = DeveloperTokenConfig(client_id=get_env_var('CLIENT_ID'), client_secret=get_env_var('CLIENT_SECRET'))
    auth: BoxDeveloperTokenAuth = BoxDeveloperTokenAuth(token='', config=developer_token_config)
...
def test_developer_token_auth_revoke():
    ...
    auth: BoxDeveloperTokenAuth = BoxDeveloperTokenAuth(token=token.access_token, config=developer_token_config)
    token_from_storage_before_revoke: AccessToken = auth.retrieve_token()
    assert not token_from_storage_before_revoke == None
    auth.revoke_token()
    with pytest.raises(Exception):
...
def test_oauth_auth_revoke():
    ...
    config: OAuthConfig = OAuthConfig(client_id=get_env_var('CLIENT_ID'), client_secret=get_env_var('CLIENT_SECRET'), token_storage=token_storage)
    auth: BoxOAuth = BoxOAuth(config=config)
