...
def testCreateUpdateGetDeleteRetentionPolicy():
    ...
    retention_policy_assignment: RetentionPolicyAssignment = client.retention_policy_assignments.create_retention_policy_assignment(retention_policy.id, CreateRetentionPolicyAssignmentAssignTo(id=folder.id, type=CreateRetentionPolicyAssignmentAssignToTypeField.FOLDER))
    ...
    assert to_string(retention_policy_assignment.assigned_to.type) == to_string(folder.type)
    ...
    new_files: Files = client.uploads.upload_file_version(file.id, UploadFileVersionAttributes(name=file.name), generate_byte_stream(20))
    new_file: FileFull = new_files.entries[0]
    assert new_file.id == file.id
    file_version_retentions: FileVersionRetentions = client.file_version_retentions.get_file_version_retentions()
    ...
    file_version_retention_by_id: FileVersionRetention = client.file_version_retentions.get_file_version_retention_by_id(file_version_retention.id)
