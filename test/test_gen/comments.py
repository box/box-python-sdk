...
def testComments():
    ...
    parent_id: str = '0'
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=file_name, parent=UploadFileAttributesParentField(id=parent_id)), file_byte_stream)
    ...
    new_comment: CommentFull = client.comments.create_comment(message, CreateCommentItem(id=file_id, type=CreateCommentItemTypeField.FILE))
    assert new_comment.message == message
    assert new_comment.is_reply_comment == False
    assert new_comment.item.id == file_id
    new_reply_comment: CommentFull = client.comments.create_comment(message, CreateCommentItem(id=new_comment.id, type=CreateCommentItemTypeField.COMMENT))
