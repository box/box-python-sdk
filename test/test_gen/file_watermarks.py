...
from box_sdk_gen.managers.file_watermarks import UpdateFileWatermarkWatermarkImprintField
...
def testCreateGetDeleteFileWatermark():
    ...
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=file_name, parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(10))
    file: FileFull = uploaded_files.entries[0]
    created_watermark: Watermark = client.file_watermarks.update_file_watermark(file.id, UpdateFileWatermarkWatermark(imprint=UpdateFileWatermarkWatermarkImprintField.DEFAULT))
