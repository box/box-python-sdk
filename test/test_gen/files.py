...
def upload_file(file_name: str, file_stream: ByteStream) -> FileFull:
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=file_name, parent=UploadFileAttributesParentField(id='0')), file_stream)
...
def testGetFileThumbnailUrl():
    ...
    thumbnail_file: FileFull = upload_file(thumbnail_file_name, thumbnail_content_stream)
    download_url: str = client.files.get_file_thumbnail_url(thumbnail_file.id, GetFileThumbnailUrlExtension.PNG)
...
def testGetFileThumbnail():
    ...
    thumbnail_content_stream: ByteStream = generate_byte_stream_from_buffer(thumbnail_buffer)
    thumbnail_file: FileFull = upload_file(thumbnail_file_name, thumbnail_content_stream)
    thumbnail: Optional[ByteStream] = client.files.get_file_thumbnail_by_id(thumbnail_file.id, GetFileThumbnailByIdExtension.PNG)
...
def testGetFileFullExtraFields():
    ...
    file: FileFull = client.files.get_file_by_id(uploaded_file.id, fields=['is_externally_owned', 'has_collaborations'])
...
def testCreateGetAndDeleteFile():
    ...
    with pytest.raises(Exception):
        client.files.get_file_by_id(uploaded_file.id, fields=['name'], extra_headers={'if-none-match': file.etag})
    assert file.name == new_file_name
    client.files.delete_file_by_id(uploaded_file.id)
    trashed_file: TrashFile = client.trashed_files.get_trashed_file_by_id(uploaded_file.id)
    assert file.id == trashed_file.id

def testUpdateFile():
    file_to_update: FileFull = upload_new_file()
    updated_name: str = get_uuid()
    updated_file: FileFull = client.files.update_file_by_id(file_to_update.id, name=updated_name, description='Updated description')
...
def testFileLock():
    ...
    file_with_lock: FileFull = client.files.update_file_by_id(file.id, lock=UpdateFileByIdLock(access=UpdateFileByIdLockAccessField.LOCK), fields=['lock'])
    assert not file_with_lock.lock == None
    file_without_lock: FileFull = client.files.update_file_by_id(file.id, lock=create_null(), fields=['lock'])
...
def testCopyFile():
    ...
    copied_file: FileFull = client.files.copy_file(file_origin.id, CopyFileParent(id='0'), name=copied_file_name)
