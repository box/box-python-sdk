...
from box_sdk_gen.schemas.search_results_with_shared_links import SearchResultsWithSharedLinks
...
from box_sdk_gen.schemas.metadata_field_filter_date_range import MetadataFieldFilterDateRange
...
def testCreateMetaDataQueryExecuteRead():
    ...
    files: Files = client.uploads.upload_file(UploadFileAttributes(name=get_uuid(), parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(10))
    file: FileFull = files.entries[0]
    metadata: MetadataFull = client.file_metadata.create_file_metadata_by_id(file.id, CreateFileMetadataByIdScope.ENTERPRISE, template_key, {'name': 'John', 'age': 23, 'birthDate': '2001-01-03T02:20:50.520Z', 'countryCode': 'US', 'sports': ['basketball', 'tennis']})
    ...
    query: MetadataQueryResults = client.search.search_by_metadata_query(search_from, '0', query='name = :name AND age < :age AND birthDate >= :birthDate AND countryCode = :countryCode AND sports = :sports', query_params={'name': 'John', 'age': 50, 'birthDate': '2001-01-01T02:20:10.120Z', 'countryCode': 'US', 'sports': ['basketball', 'tennis']})
...
def testMetadataFilters():
    ...
    files: Files = client.uploads.upload_file(UploadFileAttributes(name=get_uuid(), parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(10))
    file: FileFull = files.entries[0]
    metadata: MetadataFull = client.file_metadata.create_file_metadata_by_id(file.id, CreateFileMetadataByIdScope.ENTERPRISE, template_key, {'floatField': 10, 'stringField': 'stringValue', 'dateField': '2035-01-02T00:00:00Z', 'enumField': 'enumValue2', 'multiSelectField': ['multiSelectValue1', 'multiSelectValue2']})
    query: Union[SearchResults, SearchResultsWithSharedLinks] = client.search.search_for_content(ancestor_folder_ids=['0'], mdfilters=[MetadataFilter(filters={'stringField': 'stringValue', 'dateField': MetadataFieldFilterDateRange(lt=date_time_from_string('2035-01-01T00:00:00Z'), gt=date_time_from_string('2035-01-03T00:00:00Z')), 'floatField': MetadataFieldFilterFloatRange(lt=9.5, gt=10.5), 'enumField': 'enumValue2', 'multiSelectField': ['multiSelectValue1', 'multiSelectValue2']}, scope=MetadataFilterScopeField.ENTERPRISE, template_key=template_key)])
    assert len(query.entries) >= 0
    client.metadata_templates.delete_metadata_template(DeleteMetadataTemplateScope.ENTERPRISE, template.template_key)
    client.files.delete_file_by_id(file.id)
...
def testGetSearch():
    keyword: str = 'test'
    search: Union[SearchResults, SearchResultsWithSharedLinks] = client.search.search_for_content(query=keyword, ancestor_folder_ids=['0'], trash_content=SearchForContentTrashContent.NON_TRASHED_ONLY)
    assert len(search.entries) >= 0
    assert to_string(search.type) == 'search_results_items'
    search_with_shared_link: Union[SearchResults, SearchResultsWithSharedLinks] = client.search.search_for_content(query=keyword, ancestor_folder_ids=['0'], trash_content=SearchForContentTrashContent.NON_TRASHED_ONLY, include_recent_shared_links=True)
