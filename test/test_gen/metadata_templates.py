...
from box_sdk_gen.managers.metadata_templates import UpdateMetadataTemplateRequestBodyOpField
...
def testMetadataTemplates():
    ...
    updated_template: MetadataTemplate = client.metadata_templates.update_metadata_template(UpdateMetadataTemplateScope.ENTERPRISE, template_key, [UpdateMetadataTemplateRequestBody(op=UpdateMetadataTemplateRequestBodyOpField.ADDFIELD, field_key='newfieldname', data={'type': 'string', 'displayName': 'newFieldName'})])
    ...
    get_metadata_template: MetadataTemplate = client.metadata_templates.get_metadata_template_by_id(template.id)
    assert get_metadata_template.id == template.id
    get_metadata_template_schema: MetadataTemplate = client.metadata_templates.get_metadata_template(GetMetadataTemplateScope.ENTERPRISE, template.template_key)
    assert get_metadata_template_schema.id == template.id
    enterprise_metadata_templates: MetadataTemplates = client.metadata_templates.get_enterprise_metadata_templates()
    assert len(enterprise_metadata_templates.entries) > 0
    global_metadata_templates: MetadataTemplates = client.metadata_templates.get_global_metadata_templates()
...
def testGetMetadataTemplateByInstance():
    ...
    template_key: str = ''.join(['key', get_uuid()])
    template: MetadataTemplate = client.metadata_templates.create_metadata_template('enterprise', template_key, template_key=template_key, fields=[CreateMetadataTemplateFields(type=CreateMetadataTemplateFieldsTypeField.STRING, key='testName', display_name='testName')])
    created_metadata_instance: MetadataFull = client.file_metadata.create_file_metadata_by_id(file.id, CreateFileMetadataByIdScope.ENTERPRISE, template_key, {'testName': 'xyz'})
    metadata_templates: MetadataTemplates = client.metadata_templates.get_metadata_templates_by_instance_id(created_metadata_instance.id)
