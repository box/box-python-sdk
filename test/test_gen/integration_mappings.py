...
from box_sdk_gen.managers.user_collaborations import CreateCollaborationAccessibleByTypeField
...
from box_sdk_gen.schemas.integration_mapping_partner_item_teams_create_request import IntegrationMappingPartnerItemTeamsCreateRequestTypeField
...
def testSlackIntegrationMappings():
    ...
    folder: FolderFull = user_client.folders.create_folder(get_uuid(), CreateFolderParent(id='0'))
    user_client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FOLDER, id=folder.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.USER, id=slack_automation_user_id), CreateCollaborationRole.CO_OWNER)
    slack_integrations: IntegrationMappings = user_client.integration_mappings.get_slack_integration_mapping()
    if len(slack_integrations.entries) == 0:
        user_client.integration_mappings.create_slack_integration_mapping(IntegrationMappingPartnerItemSlack(id=slack_partner_item_id, slack_org_id=slack_org_id), IntegrationMappingBoxItemSlack(id=folder.id))
    ...
    updated_slack_mapping: IntegrationMapping = user_client.integration_mappings.update_slack_integration_mapping_by_id(slack_integration_mapping.id, box_item=IntegrationMappingBoxItemSlack(id=folder.id))
    ...
    if len(slack_mappings.entries) > 2:
        user_client.integration_mappings.delete_slack_integration_mapping_by_id(slack_integration_mapping.id)
...
def testTeamsIntegrationMappings():
    ...
    with pytest.raises(Exception):
        user_client.integration_mappings.create_teams_integration_mapping(IntegrationMappingPartnerItemTeamsCreateRequest(type=IntegrationMappingPartnerItemTeamsCreateRequestTypeField.CHANNEL, id=partner_item_id, tenant_id=tenant_id, team_id=team_id), FolderReference(id=folder.id))
    with pytest.raises(Exception):
        user_client.integration_mappings.get_teams_integration_mapping()
    integration_mapping_id: str = '123456'
    with pytest.raises(Exception):
        user_client.integration_mappings.update_teams_integration_mapping_by_id(integration_mapping_id, box_item=FolderReference(id='1234567'))
    with pytest.raises(Exception):
        user_client.integration_mappings.delete_teams_integration_mapping_by_id(integration_mapping_id)
