...
class _TestPartAccumulator:
    def __init__(self, last_index: int, parts: List[UploadPart], file_size: int, file_hash: Hash, *, upload_part_url: str = '', upload_session_id: str = ''):
...
def _reducer_by_id(acc: _TestPartAccumulator, chunk: ByteStream) -> _TestPartAccumulator:
    ...
    uploaded_part: UploadedPart = client.chunked_uploads.upload_file_part(acc.upload_session_id, generate_byte_stream_from_buffer(chunk_buffer), digest, content_range)
    ...
    return _TestPartAccumulator(last_index=bytes_end, parts=parts + [part], file_size=acc.file_size, upload_session_id=acc.upload_session_id, file_hash=acc.file_hash)
...
def testChunkedManualProcessById():
    ...
    upload_session: UploadSession = client.chunked_uploads.create_file_upload_session(parent_folder_id, file_size, file_name)
    ...
    results: _TestPartAccumulator = reduce_iterator(chunks_iterator, _reducer_by_id, _TestPartAccumulator(last_index=-1, parts=[], file_size=file_size, upload_session_id=upload_session_id, file_hash=file_hash))
    parts: List[UploadPart] = results.parts
    processed_session_parts: UploadParts = client.chunked_uploads.get_file_upload_session_parts(upload_session_id)
    assert processed_session_parts.total_count == total_parts
    processed_session: UploadSession = client.chunked_uploads.get_file_upload_session_by_id(upload_session_id)
    assert processed_session.id == upload_session_id
    sha_1: str = file_hash.digest_hash('base64')
    digest: str = ''.join(['sha=', sha_1])
    committed_session: Optional[Files] = client.chunked_uploads.create_file_upload_session_commit(upload_session_id, parts, digest)
...
def _reducer_by_url(acc: _TestPartAccumulator, chunk: ByteStream) -> _TestPartAccumulator:
    last_index: int = acc.last_index
    parts: List[UploadPart] = acc.parts
    chunk_buffer: Buffer = read_byte_stream(chunk)
    hash: Hash = Hash(algorithm=HashName.SHA1)
    ...
    bytes_end: int = last_index + chunk_size
    content_range: str = ''.join(['bytes ', to_string(bytes_start), '-', to_string(bytes_end), '/', to_string(acc.file_size)])
    uploaded_part: UploadedPart = client.chunked_uploads.upload_file_part_by_url(acc.upload_part_url, generate_byte_stream_from_buffer(chunk_buffer), digest, content_range)
    part: UploadPart = uploaded_part.part
    part_sha_1: str = hex_to_base_64(part.sha_1)
    assert part_sha_1 == sha_1
    assert part.size == chunk_size
    assert part.offset == bytes_start
    acc.file_hash.update_hash(chunk_buffer)
    return _TestPartAccumulator(last_index=bytes_end, parts=parts + [part], file_size=acc.file_size, upload_part_url=acc.upload_part_url, file_hash=acc.file_hash)
...
def testChunkedManualProcessByUrl():
    ...
    file_hash: Hash = Hash(algorithm=HashName.SHA1)
    chunks_iterator: Iterator = iterate_chunks(file_byte_stream, part_size, file_size)
    results: _TestPartAccumulator = reduce_iterator(chunks_iterator, _reducer_by_url, _TestPartAccumulator(last_index=-1, parts=[], file_size=file_size, upload_part_url=upload_part_url, file_hash=file_hash))
    parts: List[UploadPart] = results.parts
    processed_session_parts: UploadParts = client.chunked_uploads.get_file_upload_session_parts_by_url(list_parts_url)
    assert processed_session_parts.total_count == total_parts
    processed_session: UploadSession = client.chunked_uploads.get_file_upload_session_by_url(status_url)
    assert processed_session.id == upload_session_id
    sha_1: str = file_hash.digest_hash('base64')
    digest: str = ''.join(['sha=', sha_1])
    committed_session: Optional[Files] = client.chunked_uploads.create_file_upload_session_commit_by_url(commit_url, parts, digest)
    assert committed_session.entries[0].name == file_name
...
def testChunkedUploadConvenienceMethod():
    ...
    file_name: str = get_uuid()
    parent_folder_id: str = '0'
    uploaded_file: File = client.chunked_uploads.upload_big_file(file_byte_stream, file_name, file_size, parent_folder_id)
