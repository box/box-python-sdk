...
def testMakeRequestJsonCRUD():
    ...
    request_body_post: str = ''.join(['{"name": "', new_folder_name, '", "parent": { "id": "0"}}'])
    create_folder_response: FetchResponse = client.make_request(FetchOptions(method='post', url='https://api.box.com/2.0/folders', data=json_to_serialized_data(request_body_post)))
    ...
    update_folder_response: FetchResponse = client.make_request(FetchOptions(method='put', url=''.join(['https://api.box.com/2.0/folders/', get_sd_value_by_key(created_folder, 'id')]), data=json_to_serialized_data(request_body_put)))
    ...
    assert get_sd_value_by_key(updated_folder, 'id') == get_sd_value_by_key(created_folder, 'id')
    get_folder_response: FetchResponse = client.make_request(FetchOptions(url=''.join(['https://api.box.com/2.0/folders/', get_sd_value_by_key(created_folder, 'id')]), method='GET'))
    ...
    assert get_sd_value_by_key(received_folder, 'id') == get_sd_value_by_key(updated_folder, 'id')
    delete_folder_response: FetchResponse = client.make_request(FetchOptions(url=''.join(['https://api.box.com/2.0/folders/', get_sd_value_by_key(received_folder, 'id')]), method='DELETE'))
    assert delete_folder_response.status == 204

def testMakeRequestMultipart():
    new_folder_name: str = get_uuid()
    new_folder: FolderFull = client.folders.create_folder(new_folder_name, CreateFolderParent(id='0'))
    ...
    multipart_attributes: str = ''.join(['{"name": "', new_file_name, '", "parent": { "id":', new_folder_id, '}}'])
    upload_file_response: FetchResponse = client.make_request(FetchOptions(method='POST', url='https://upload.box.com/api/2.0/files/content', content_type='multipart/form-data', multipart_data=[MultipartItem(part_name='attributes', data=json_to_serialized_data(multipart_attributes)), MultipartItem(part_name='file', file_stream=file_content_stream)]))
...
def testMakeRequestBinaryFormat():
    ...
    uploaded_files: Files = client.uploads.upload_file(UploadFileAttributes(name=new_file_name, parent=UploadFileAttributesParentField(id='0')), file_content_stream)
    uploaded_file: FileFull = uploaded_files.entries[0]
    download_file_response: FetchResponse = client.make_request(FetchOptions(method='GET', url=''.join(['https://api.box.com/2.0/files/', uploaded_file.id, '/content']), response_format=ResponseFormat.BINARY))
...
def testWithAsUserHeader():
    ...
    created_user: UserFull = client.users.create_user(user_name, is_platform_access_only=True)
...
def testWithExtraHeaders():
    user_name: str = get_uuid()
    created_user: UserFull = client.users.create_user(user_name, is_platform_access_only=True)
    as_user_client: BoxClient = client.with_extra_headers(extra_headers={'As-User': created_user.id})
    admin_user: UserFull = client.users.get_user_me()
    assert not to_string(admin_user.name) == user_name
    app_user: UserFull = as_user_client.users.get_user_me()
    assert to_string(app_user.name) == user_name
...
def testWithCustomBaseUrls():
    new_base_urls: BaseUrls = BaseUrls(base_url='https://box.com/', upload_url='https://box.com/', oauth_2_url='https://box.com/')
