...
def testAiStudioCRUD():
    ...
    created_agent: AiSingleAgentResponseFull = client.ai_studio.create_ai_agent(agent_name, 'enabled', ask=AiStudioAgentAsk(access_state='enabled', description='desc1'))
    ...
    retrieved_agent: AiSingleAgentResponseFull = client.ai_studio.get_ai_agent_by_id(created_agent.id, fields=['ask'])
    ...
    updated_agent: AiSingleAgentResponseFull = client.ai_studio.update_ai_agent_by_id(created_agent.id, agent_name, 'enabled', ask=AiStudioAgentAsk(access_state='disabled', description='desc2'))
...
def testUseAIAgentReferenceInAIAsk():
    ...
    response: Optional[AiResponseFull] = client.ai.create_ai_ask(CreateAiAskMode.SINGLE_ITEM_QA, 'which direction sun rises', [AiItemAsk(id=file_to_ask.id, type=AiItemAskTypeField.FILE, content='Sun rises in the East')], ai_agent=AiAgentReference(id=created_agent.id))
