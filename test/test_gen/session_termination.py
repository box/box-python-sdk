...
def testSessionTerminationUser():
    ...
    result: SessionTerminationMessage = client.session_termination.terminate_users_sessions([get_env_var('USER_ID')], [user.login])
    assert result.message == 'Request is successful, please check the admin events for the status of the job'

def testSessionTerminationGroup():
    group_name: str = get_uuid()
    group: GroupFull = client.groups.create_group(group_name)
    result: SessionTerminationMessage = client.session_termination.terminate_groups_sessions([group.id])
    assert result.message == 'Request is successful, please check the admin events for the status of the job'
