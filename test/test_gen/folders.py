...
def test_get_folder_full_info_with_extra_fields():
    root_folder: FolderFull = client.folders.get_folder_by_id('0', fields=['has_collaborations', 'tags'])
    assert root_folder.id == '0'
...
def test_update_folder():
    ...
    folder_to_update: FolderFull = client.folders.create_folder(folder_to_update_name, CreateFolderParent(id='0'))
    updated_name: str = get_uuid()
    updated_folder: FolderFull = client.folders.update_folder_by_id(folder_to_update.id, name=updated_name, description='Updated description')
...
def test_copy_move_folder_and_list_folder_items():
    ...
    folder_origin: FolderFull = client.folders.create_folder(folder_origin_name, CreateFolderParent(id='0'))
    copied_folder_name: str = get_uuid()
    copied_folder: FolderFull = client.folders.copy_folder(folder_origin.id, CopyFolderParent(id='0'), name=copied_folder_name)
    assert copied_folder.parent.id == '0'
    moved_folder_name: str = get_uuid()
    moved_folder: FolderFull = client.folders.update_folder_by_id(copied_folder.id, name=moved_folder_name, parent=UpdateFolderByIdParent(id=folder_origin.id))
