...
def testWorkflows():
    ...
    uploaded_files: Files = admin_client.uploads.upload_file(UploadFileAttributes(name=get_uuid(), parent=UploadFileAttributesParentField(id=workflow_folder_id)), generate_byte_stream(1024 * 1024))
    ...
    assert to_string(workflow_to_run.flows[0].trigger.trigger_type) == 'WORKFLOW_MANUAL_START'
