...
def testMemberships():
    user: UserFull = client.users.create_user(get_uuid(), login=''.join([get_uuid(), '@boxdemo.com']))
    user_memberships: GroupMemberships = client.memberships.get_user_memberships(user.id)
    assert user_memberships.total_count == 0
    group: GroupFull = client.groups.create_group(get_uuid())
    group_memberships: GroupMemberships = client.memberships.get_group_memberships(group.id)
    assert group_memberships.total_count == 0
    group_membership: GroupMembership = client.memberships.create_group_membership(CreateGroupMembershipUser(id=user.id), CreateGroupMembershipGroup(id=group.id))
    ...
    get_group_membership: GroupMembership = client.memberships.get_group_membership_by_id(group_membership.id)
    assert get_group_membership.id == group_membership.id
    updated_group_membership: GroupMembership = client.memberships.update_group_membership_by_id(group_membership.id, role=UpdateGroupMembershipByIdRole.ADMIN)
