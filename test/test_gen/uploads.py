...
def testUploadFileAndFileVersion():
    ...
    uploaded_files_version: Files = client.uploads.upload_file_version(uploaded_file.id, UploadFileVersionAttributes(name=new_file_version_name), new_file_content_stream)
...
def testUploadFileWithPreflightCheck():
    ...
    upload_files_with_preflight: Files = client.uploads.upload_with_preflight_check(UploadWithPreflightCheckAttributes(name=new_file_name, size=1024 * 1024, parent=UploadWithPreflightCheckAttributesParentField(id='0')), file_content_stream)
    file: FileFull = upload_files_with_preflight.entries[0]
    assert file.name == new_file_name
    assert file.size == 1024 * 1024
    with pytest.raises(Exception):
        client.uploads.upload_with_preflight_check(UploadWithPreflightCheckAttributes(name=new_file_name, size=1024 * 1024, parent=UploadWithPreflightCheckAttributesParentField(id='0')), file_content_stream)
    client.files.delete_file_by_id(file.id)

def testPreflightCheck():
    new_file_name: str = get_uuid()
    preflight_check_result: UploadUrl = client.uploads.preflight_file_upload_check(name=new_file_name, size=1024 * 1024, parent=PreflightFileUploadCheckParent(id='0'))
