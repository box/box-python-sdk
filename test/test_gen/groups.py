...
def test_create_get_delete_group():
    ...
    group: GroupFull = client.groups.create_group(group_name, description=group_description)
    assert group.name == group_name
    group_by_id: GroupFull = client.groups.get_group_by_id(group.id, fields=['id', 'name', 'description', 'group_type'])
    assert group_by_id.id == group.id
    assert group_by_id.description == group_description
    updated_group_name: str = get_uuid()
    updated_group: GroupFull = client.groups.update_group_by_id(group.id, name=updated_group_name)
