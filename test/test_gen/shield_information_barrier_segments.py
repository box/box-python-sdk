...
def testShieldInformationBarrierSegments():
    ...
    segment: ShieldInformationBarrierSegment = client.shield_information_barrier_segments.create_shield_information_barrier_segment(ShieldInformationBarrierBase(id=barrier_id, type=ShieldInformationBarrierBaseTypeField.SHIELD_INFORMATION_BARRIER), segment_name, description=segment_description)
    assert segment.name == segment_name
    segments: ShieldInformationBarrierSegments = client.shield_information_barrier_segments.get_shield_information_barrier_segments(barrier_id)
    assert len(segments.entries) > 0
    segment_id: str = segment.id
    segment_from_api: ShieldInformationBarrierSegment = client.shield_information_barrier_segments.get_shield_information_barrier_segment_by_id(segment_id)
    ...
    updated_segment: ShieldInformationBarrierSegment = client.shield_information_barrier_segments.update_shield_information_barrier_segment_by_id(segment_id, description=updated_segment_description)
