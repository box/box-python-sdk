...
def testCreateListGetPromoteFileVersion():
    ...
    files: Files = client.uploads.upload_file(UploadFileAttributes(name=old_name, parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(10))
    file: FileFull = files.entries[0]
    assert file.name == old_name
    assert file.size == 10
    new_files: Files = client.uploads.upload_file_version(file.id, UploadFileVersionAttributes(name=new_name), generate_byte_stream(20))
    ...
    file_version: FileVersionFull = client.file_versions.get_file_version_by_id(file.id, file_versions.entries[0].id)
...
def testRemoveAndRestoreFileVersion():
    ...
    files: Files = client.uploads.upload_file(UploadFileAttributes(name=old_name, parent=UploadFileAttributesParentField(id='0')), generate_byte_stream(10))
    file: FileFull = files.entries[0]
    client.uploads.upload_file_version(file.id, UploadFileVersionAttributes(name=new_name), generate_byte_stream(20))
    file_versions: FileVersions = client.file_versions.get_file_versions(file.id)
    assert file_versions.total_count == 1
    file_version: FileVersionFull = client.file_versions.get_file_version_by_id(file.id, file_versions.entries[0].id, fields=['trashed_at', 'trashed_by', 'restored_at', 'restored_by'])
    ...
    client.file_versions.delete_file_version_by_id(file.id, file_version.id)
    deleted_file_version: FileVersionFull = client.file_versions.get_file_version_by_id(file.id, file_versions.entries[0].id, fields=['trashed_at', 'trashed_by', 'restored_at', 'restored_by'])
    ...
    restored_file_version: FileVersionFull = client.file_versions.get_file_version_by_id(file.id, file_versions.entries[0].id, fields=['trashed_at', 'trashed_by', 'restored_at', 'restored_by'])
