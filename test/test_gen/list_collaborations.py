...
def testListCollaborations():
    ...
    group_collaboration: Collaboration = client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FOLDER, id=folder.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.GROUP, id=group.id), CreateCollaborationRole.EDITOR)
    file_collaboration: Collaboration = client.user_collaborations.create_collaboration(CreateCollaborationItem(type=CreateCollaborationItemTypeField.FILE, id=file.id), CreateCollaborationAccessibleBy(type=CreateCollaborationAccessibleByTypeField.USER, id=get_env_var('USER_ID')), CreateCollaborationRole.EDITOR)
    assert to_string(group_collaboration.role) == 'editor'
    assert to_string(group_collaboration.type) == 'collaboration'
    file_collaborations: Collaborations = client.list_collaborations.get_file_collaborations(file.id)
    ...
    pending_collaborations: CollaborationsOffsetPaginated = client.list_collaborations.get_collaborations(GetCollaborationsStatus.PENDING)
    assert len(pending_collaborations.entries) >= 0
    group_collaborations: CollaborationsOffsetPaginated = client.list_collaborations.get_group_collaborations(group.id)
