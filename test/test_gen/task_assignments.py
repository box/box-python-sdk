...
from box_sdk_gen.managers.task_assignments import UpdateTaskAssignmentByIdResolutionState
...
def testCreateUpdateGetDeleteTaskAssignment():
    ...
    task: Task = client.tasks.create_task(CreateTaskItem(type=CreateTaskItemTypeField.FILE, id=file.id), action=CreateTaskAction.REVIEW, message='test message', due_at=date, completion_rule=CreateTaskCompletionRule.ALL_ASSIGNEES)
    assert task.message == 'test message'
    assert task.item.id == file.id
    current_user: UserFull = client.users.get_user_me()
    task_assignment: TaskAssignment = client.task_assignments.create_task_assignment(CreateTaskAssignmentTask(type=CreateTaskAssignmentTaskTypeField.TASK, id=task.id), CreateTaskAssignmentAssignTo(id=current_user.id))
    assert task_assignment.item.id == file.id
    assert task_assignment.assigned_to.id == current_user.id
    task_assignment_by_id: TaskAssignment = client.task_assignments.get_task_assignment_by_id(task_assignment.id)
    assert task_assignment_by_id.id == task_assignment.id
    task_assignments_on_task: TaskAssignments = client.task_assignments.get_task_assignments(task.id)
    assert task_assignments_on_task.total_count == 1
    updated_task_assignment: TaskAssignment = client.task_assignments.update_task_assignment_by_id(task_assignment.id, message='updated message', resolution_state=UpdateTaskAssignmentByIdResolutionState.APPROVED)
