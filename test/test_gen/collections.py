...
def testCollections():
    ...
    favourite_collection: Collection = client.collections.get_collection_by_id(collections.entries[0].id)
    assert to_string(favourite_collection.type) == 'collection'
    assert to_string(favourite_collection.collection_type) == 'favorites'
    collection_items: ItemsOffsetPaginated = client.collections.get_collection_items(favourite_collection.id)
    folder: FolderFull = client.folders.create_folder(get_uuid(), CreateFolderParent(id='0'))
    client.folders.update_folder_by_id(folder.id, collections=[UpdateFolderByIdCollections(id=favourite_collection.id)])
    collection_items_after_update: ItemsOffsetPaginated = client.collections.get_collection_items(favourite_collection.id)
    assert collection_items_after_update.total_count > 0
    client.folders.update_folder_by_id(folder.id, collections=[])
    collection_items_after_remove: ItemsOffsetPaginated = client.collections.get_collection_items(favourite_collection.id)
