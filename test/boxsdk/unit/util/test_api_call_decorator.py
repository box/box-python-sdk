...
def mock_cloneable(cloneable_subclass_with_api_call_method):
    ...
    return MockCloneable(
        spec_set=cloneable_subclass_with_api_call_method, name='Cloneable'
    )
...
def test_api_call_decorated_method_must_be_bound_to_an_instance_of_the_owner(
    mock_cloneable, api_call_method
):
    # pylint:disable=abstract-method
...
def test_api_call_decorated_method_returns_itself_when_bound_to_none(
    ...
    assert not hasattr(
        cloneable_subclass_with_api_call_method.api_call_method, '__self__'
    )
...
def test_api_call_decorated_method_delegates_to_wrapped_method(
    mock_cloneable, api_call_result
):
    args = (1, 2, 'ƒøø', 'bar')
    kwargs = {'bar': 'ƒøø'}
    assert mock_cloneable.api_call_method(*args, **kwargs) == (
        mock_cloneable,
        args,
        kwargs,
        api_call_result,
...
def test_api_call_decorated_method_can_be_called_as_an_unbound_method_with_an_instance_as_the_first_argument(
    ...
    assert api_call_method(mock_cloneable, *args, **kwargs) == (
        mock_cloneable,
        args,
        kwargs,
        api_call_result,
