...
def test_successful_create_ccg_auth_object_and_normalize_user_id(
    client_id, client_secret, enterprise_id, user, expected_normalized_user_id
):
    auth = CCGAuth(
        client_id=client_id,
        client_secret=client_secret,
...
def test_throw_type_error_when_invalid_user_object_provided(
    client_id, client_secret, mock_enterprise_id
):
...
def test_authenticate_user_success(ccg_auth, user, mock_user_id):
    with mock.patch.object(
        CCGAuth, '_fetch_access_token', return_value='new_token'
    ) as fetch_access_token_mock:
        ccg_auth.authenticate_user(user)

        fetch_access_token_mock.assert_called_once_with(
            mock_user_id, USER_SUBJECT_TYPE, None
        )
...
@pytest.mark.parametrize(
    'error_description',
    ('invalid box_sub_type claim', 'invalid kid', "check the 'exp' claim"),
)
...
def test_auth_retry_for_invalid_exp_claim(
    ...
    side_effect = [
        BoxOAuthException(status_code, network_response=unsuccessful_ccg_response),
        'jwt_token',
    ]
    with mock.patch.object(
        CCGAuth,
...
def test_auth_retry_for_rate_limit_and_server_errors(
    unsuccessful_ccg_response, ccg_auth, status_code, mock_enterprise_id
):
...
def test_extract_token_from_success_response(
    ...
    with mock.patch.object(
        CCGAuth, '_execute_token_request', return_value=successful_token_response
    ):
...
def test_send_authentication_request_with_correct_params(
    ccg_auth, client_id, client_secret, subject_id, subject_type
):
    expected_data = {
    ...
    ccg_auth.send_token_request.assert_called_once_with(
        expected_data, access_token=None, expect_refresh_token=False
    )
