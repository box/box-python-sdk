...
class BoxObjectCollectionTestBase(ABC):
    ...
    def test_new_object_collection_starts_off_from_last_pointer(
        self, mock_session, entries
    ):
        """
        ...
        iterated_items = []
        object_collection = self._object_collection_instance(
            mock_session, limit=5, return_full_pages=True
        )
        ...
        new_object_collection = self._object_collection_instance(
            mock_session, limit=5, return_full_pages=True, starting_pointer=next_pointer
        )
