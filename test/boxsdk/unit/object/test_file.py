...
def mock_accelerator_response_for_update(
    make_mock_box_request, mock_accelerator_upload_url_for_update
):
...
def test_get_download_url(test_file, mock_box_session):
    ...
    mock_box_session.get.return_value.headers = {'location': download_url}
    url = test_file.get_download_url()
    mock_box_session.get.assert_called_once_with(
        expected_url, params=None, expect_json_response=False, allow_redirects=False
    )
...
@pytest.mark.parametrize(
    'params,expected_query,expected_headers',
    [
        ({}, None, None),
...
def test_download_to_file_version(
    test_file, test_file_version, mock_box_session, mock_content_response
):
    expected_url = f'{API.BASE_API_URL}/files/{test_file.object_id}/content'
    mock_box_session.get.return_value = mock_content_response
    mock_writeable_stream = BytesIO()
...
def test_get_content_file_version(
    test_file, mock_box_session, mock_content_response, test_file_version
):
    expected_url = test_file.get_url('content')
    mock_box_session.get.return_value = mock_content_response
...
@pytest.mark.parametrize(
    ...
    [
        ...
        datetime(
            1970, 1, 1, 11, 11, 11, microsecond=500, tzinfo=pytz.timezone('US/Alaska')
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {'is_download_prevented': False, 'expires_at': '2018-11-06T19:40:00-08:00'},
        ),
        (
            {
                'expire_time': datetime(
                    2018, 11, 6, 19, 40, 00, tzinfo=pytz.timezone('US/Alaska')
                )
            },
            {'is_download_prevented': False, 'expires_at': '2018-11-06T19:40:00+14:00'},
        ),
    ],
)
def test_lock(test_file, mock_box_session, mock_file_response, params, expected_data):
    expected_url = test_file.get_url()
    expected_body = {'lock': {'type': 'lock'}}

    if 'is_download_prevented' in expected_data.keys():
        expected_body['lock']['is_download_prevented'] = expected_data[
...
def test_get_comments(test_file, mock_box_session):
    ...
    mock_comment1 = {'type': 'comment', 'id': '11111', 'message': 'Foo'}
    mock_comment2 = {
        'type': 'comment',
        'id': '22222',
...
def test_delete_version(
    test_file, test_file_version, mock_box_session, params, expected_headers
):
    ...
    mock_box_session.delete.assert_called_once_with(
        expected_url, expect_json_response=False, headers=expected_headers
    )
...
def test_get_embed_url(test_file, mock_box_session):
    ...
    expected_params = {'fields': 'expiring_embed_link'}
...
@pytest.mark.parametrize(
    'rep_hints,expected_headers',
    [
        (None, None),
        ('[pdf]', {'X-Rep-Hints': '[pdf]'}),
    ],
)
def test_get_representation_info(
    test_file, mock_box_session, rep_hints, expected_headers
):
    expected_url = f'{API.BASE_API_URL}/files/{test_file.object_id}'
...
@pytest.mark.parametrize(
    'extension,min_width,min_height,max_width,max_height,expected_params',
    [
...
def test_get_thumbnail(
    ...
    expected_url = (
        f'{API.BASE_API_URL}/files/{test_file.object_id}/thumbnail.{extension}'
    )
    mock_box_session.get.return_value = mock_content_response
    ...
    mock_box_session.get.assert_called_once_with(
        expected_url, expect_json_response=False, params=expected_params
    )
...
def test_get_thumbnail_representation(
    ...
    mock_representations_response.json.return_value = {
        ...
        'representations': {
            'entries': [
                {
                    'content': {'url_template': content_url + '{+asset_path}'},
                    'info': {
                        'url': (
                            'https://api.box.com/2.0/internal_files/123/versions/345/representations/jpg'
                        )
                    },
                    'properties': {},
                    'representation': 'pdf',
    ...
    mock_box_session.get.assert_any_call(
        representation_url,
        headers={'X-Rep-Hints': f'[{extension}?dimensions=92x92]'},
