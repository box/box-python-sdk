...
def test_get(test_terms_of_service, mock_box_session):
    ...
    expected_url = (
        f'{API.BASE_API_URL}/terms_of_services/{test_terms_of_service.object_id}'
    )
...
def test_accept_terms_of_service(
    ...
    assert (
        new_terms_of_service_user_status.id
        == test_terms_of_service_user_status.object_id
    )
    assert (
        new_terms_of_service_user_status.type
        == test_terms_of_service_user_status.object_type
    )
    assert new_terms_of_service_user_status.created_at == created_at


def test_reject_terms_of_service(
    test_terms_of_service,
    test_terms_of_service_user_status,
    mock_user,
    mock_box_session,
...
def test_set_user_status(test_terms_of_service, mock_user, mock_box_session):
    ...
    mock_box_session.put.assert_called_once_with(
        expected_put_url, data=put_value, headers=None, params=None
    )
    assert isinstance(new_terms_of_service_user_status, TermsOfServiceUserStatus)
    assert new_terms_of_service_user_status.type == 'terms_of_service_user_status'
    assert new_terms_of_service_user_status.id == '12345'
    assert (
        new_terms_of_service_user_status.tos['type']
        == test_terms_of_service.object_type
    )
