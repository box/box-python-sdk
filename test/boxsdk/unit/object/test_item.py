...
def test_item_and_response(
    test_file, test_folder, mock_file_response, mock_folder_response, request
):
...
def test_update_info(test_item_and_response, mock_box_session, etag, if_match_header):
    ...
    mock_box_session.put.assert_called_once_with(
        expected_url, data=json.dumps(data), headers=if_match_header, params=None
    )
    assert isinstance(update_response, test_item.__class__)
    assert update_response.object_id == test_item.object_id


def test_update_info_with_default_request_kwargs(
    test_item_and_response, mock_box_session, mock_box_session_2
):
    # pylint:disable=redefined-outer-name, protected-access
    test_item, mock_item_response = test_item_and_response
    expected_url = test_item.get_url()
    mock_box_session.with_default_network_request_kwargs.return_value = (
        mock_box_session_2
    )
    mock_box_session_2.put.return_value = mock_item_response
    data = {'foo': 'bar', 'baz': {'foo': 'bar'}, 'num': 4}
    extra_network_parameters = {'timeout': 1}
    update_response = test_item.update_info(
        data=data, extra_network_parameters=extra_network_parameters
    )
    mock_box_session.with_default_network_request_kwargs.assert_called_once_with(
        {'timeout': 1}
    )
    mock_box_session_2.put.assert_called_once_with(
...
def test_remove_shared_link(
    test_item_and_response, mock_box_session, etag, if_match_header
):
    # pylint:disable=redefined-outer-name, protected-access
    test_item, _ = test_item_and_response
    expected_url = test_item.get_url()
...
def test_get(
    ...
    mock_box_session.get.assert_called_once_with(
        expected_url, params=expected_params, headers=if_none_match_header
    )
...
def test_get_watermark(test_item_and_response, mock_box_session):
    ...
    expected_url = (
        f'{API.BASE_API_URL}/{test_item.object_type}s/{test_item.object_id}/watermark'
    )
...
def test_apply_watermark(test_item_and_response, mock_box_session):
    ...
    mock_box_session.put.assert_called_once_with(
        expected_url, data='{"watermark": {"imprint": "default"}}'
    )
    assert isinstance(watermark, Watermark)
    assert watermark['created_at'] == created_at
    assert watermark['modified_at'] == modified_at
...
def test_collaborate_with_group(test_item_and_response, test_group, mock_box_session):
    ...
    mock_box_session.post.assert_called_once_with(
        expected_url, data=json.dumps(expected_data), params={}
    )
...
@pytest.mark.parametrize(
    'can_view_path,fields,notify,data,params',
    [
        (None, None, None, {}, {}),
        ...
        (
            ...
            False,
            {'can_view_path': True},
            {'fields': 'type,id,created_by', 'notify': False},
        ),
    ],
)
def test_collaborate_with_user(
    test_item_and_response,
    mock_user,
    mock_box_session,
    can_view_path,
    ...
    mock_box_session.post.return_value.json.return_value = mock_collaboration
    collaboration = test_item.collaborate(
        mock_user, 'editor', can_view_path=can_view_path, fields=fields, notify=notify
    )
    mock_box_session.post.assert_called_once_with(
        expected_url, data=json.dumps(expected_data), params=expected_params
    )
    assert collaboration.id == mock_collaboration['id']
    assert collaboration['type'] == mock_collaboration['type']
    assert collaboration['created_by']['id'] == mock_collaboration['created_by']['id']
...
def test_collaborate_with_login(
    test_item_and_response,
    mock_box_session,
    can_view_path,
    fields,
    ...
    expected_params = params
    mock_box_session.post.return_value.json.return_value = mock_collaboration
    collaboration = test_item.collaborate_with_login(
        'test@example.com',
        'editor',
...
def test_get_all_metadata(test_item_and_response, mock_box_session):
    test_item, _ = test_item_and_response
    expected_url = (
        f'{API.BASE_API_URL}/{test_item.object_type}s/{test_item.object_id}/metadata'
    )
    mock_metadata = {
...
def test_add_classification(test_item_and_response, mock_box_session):
    ...
    metadata_response = mock_box_session.post.return_value.json.return_value = (
        metadata_response
    )
    data = {'Box__Security__Classification__Key': 'Public'}
    headers = {b'Content-Type': b'application/json'}
    metadata = test_item.add_classification('Public')
    mock_box_session.post.assert_called_once_with(
        expected_url, headers=headers, data=json.dumps(data)
    )
...
def test_update_classification(test_item_and_response, mock_box_session):
    ...
    metadata_response = mock_box_session.put.return_value.json.return_value = (
        metadata_response
    )
    data = [
        {
...
def test_set_classification_succeeds(test_item_and_response, mock_box_session):
    ...
    mock_box_session.post.assert_called_once_with(
        expected_url, data=json.dumps(post_data), headers=post_headers
    )
    mock_box_session.put.assert_called_once_with(
        expected_url, data=json.dumps(put_data), headers=put_headers
    )
    assert metadata is metadata_response['Box__Security__Classification__Key']
...
def test_get_classification_succeeds(test_item_and_response, mock_box_session):
    ...
    metadata_response = {'Box__Security__Classification__Key': 'Public'}
...
def test_remove_classification(
    test_item_and_response, mock_box_session, make_mock_box_request
):
    # pylint:disable=redefined-outer-name
    test_item, _ = test_item_and_response
    expected_url = (
    ...
    mock_box_session.delete.return_value, _ = make_mock_box_request(
        response_ok='success'
    )
