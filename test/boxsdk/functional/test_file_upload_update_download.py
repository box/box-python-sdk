...
from test.boxsdk.util.streamable_mock_open import streamable_mock_open
import pytest
...
def test_upload_then_update(
    box_client, test_file_path, test_file_content, update_file_content, file_name
):
    with patch(
    ...
    expected_file_content = (
        ...
        if isinstance(test_file_content, str)
    ...
    with patch(
        'boxsdk.object.file.open',
        streamable_mock_open(read_data=update_file_content),
        create=True,
    ):
    ...
    expected_file_content = (
        update_file_content.encode('utf-8')
        if isinstance(update_file_content, str)
...
def test_do_not_log_downloaded_file_content_stream(
    box_client, test_file_path, test_file_content, file_name, logger
):
    with patch(
        'boxsdk.object.folder.open',
        streamable_mock_open(read_data=test_file_content),
    ...
    assert writeable_stream.getvalue() == expected_file_content
    assert (
        logger.info.call_args[0][1]['content']
        == DefaultNetworkResponse.CONTENT_NOT_LOGGED
    )


def test_do_not_log_content_of_downloaded_file(
    box_client, test_file_path, test_file_content, file_name, logger
):
    with patch(
        'boxsdk.object.folder.open',
