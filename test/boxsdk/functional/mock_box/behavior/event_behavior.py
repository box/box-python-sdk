...
class EventBehavior:
    ...
    def get_events(self):
        ...
        max_stream_position = (
            max(e.stream_position for e in results) if results else self._epoch
        )
        next_stream_position = (
            datetime.utcnow()
            if total_count < limit
            else max_stream_position - timedelta(seconds=5)
        )
        ...
        file_records = (
            ...
            .filter(FileModel.file_id.in_(file_ids))
            .all()
            if file_ids
            else []
        )
        folder_records = (
            ...
            .filter(FolderModel.folder_id.in_(folder_ids))
            .all()
            if folder_ids
            else []
        )
            ...
            results = [
                {
                    ...
                    'source': (
                        next(
                            (f for f in file_records if f.file_id == e.source_id), None
                        )
                        or next(
            ...
            'entries': [
                {
                    ...
                    'url': (
                        'http://localhost:18089/subscribe?channel=cc807c9c4869ffb1c81a&stream_type=all'
                    ),
                    'ttl': 10,
                    'max_retries': 10,
                    'retry_timeout': 610,
