...
from box_sdk_gen.managers.classifications import AddClassificationRequestBodyDataStaticConfigField

from box_sdk_gen.managers.classifications import AddClassificationRequestBodyDataStaticConfigClassificationField
...
def get_ccg_auth() -> BoxCCGAuth:
    ccg_config: CCGConfig = CCGConfig(client_id=get_env_var('CLIENT_ID'), client_secret=get_env_var('CLIENT_SECRET'), enterprise_id=get_env_var('ENTERPRISE_ID'))
...
def get_default_client() -> BoxClient:
    client: BoxClient = BoxClient(auth=get_ccg_auth() if is_browser() else get_jwt_auth())
    return client
...
def get_or_create_terms_of_services() -> TermsOfService:
    ...
    if number_of_tos >= 2:
        ...
        if to_string(second_tos.tos_type) == 'managed':
            return second_tos
    return client.terms_of_services.create_terms_of_service(CreateTermsOfServiceStatus.DISABLED, 'Test TOS', tos_type=CreateTermsOfServiceTosType.MANAGED)

def get_or_create_classification(classification_template: ClassificationTemplate) -> ClassificationTemplateFieldsOptionsField:
    client: BoxClient = get_default_client()
    classifications: List[ClassificationTemplateFieldsOptionsField] = classification_template.fields[0].options
    current_number_of_classifications: int = len(classifications)
    if current_number_of_classifications == 0:
        classification_template_with_new_classification: ClassificationTemplate = client.classifications.add_classification([AddClassificationRequestBody(data=AddClassificationRequestBodyDataField(key=get_uuid(), static_config=AddClassificationRequestBodyDataStaticConfigField(classification=AddClassificationRequestBodyDataStaticConfigClassificationField(color_id=3, classification_definition='Some description'))))])
...
def get_or_create_classification_template() -> ClassificationTemplate:
    ...
    try:
        ...
        return client.classifications.create_classification_template([CreateClassificationTemplateFields(options=[])])

def get_or_create_shield_information_barrier(client: BoxClient, enterprise_id: str) -> ShieldInformationBarrier:
    barriers: ShieldInformationBarriers = client.shield_information_barriers.get_shield_information_barriers()
    number_of_barriers: int = len(barriers.entries)
    if number_of_barriers == 0:
        return client.shield_information_barriers.create_shield_information_barrier(EnterpriseBase(id=enterprise_id))
