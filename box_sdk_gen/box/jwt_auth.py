...
class JwtConfigAppSettingsAppAuth(BaseObject):
    _fields_to_json_mapping: Dict[str, str] = {'public_key_id': 'publicKeyID', 'private_key': 'privateKey', **BaseObject._fields_to_json_mapping}
...
class JwtConfigAppSettings(BaseObject):
    _fields_to_json_mapping: Dict[str, str] = {'client_id': 'clientID', 'client_secret': 'clientSecret', 'app_auth': 'appAuth', **BaseObject._fields_to_json_mapping}
    _json_to_fields_mapping: Dict[str, str] = {'clientID': 'client_id', 'clientSecret': 'client_secret', 'appAuth': 'app_auth', **BaseObject._json_to_fields_mapping}
    def __init__(self, client_id: str, client_secret: str, app_auth: JwtConfigAppSettingsAppAuth, **kwargs):
        """
...
class JwtConfigFile(BaseObject):
    _fields_to_json_mapping: Dict[str, str] = {'enterprise_id': 'enterpriseID', 'user_id': 'userID', 'box_app_settings': 'boxAppSettings', **BaseObject._fields_to_json_mapping}
...
class JWTConfig:
    def __init__(self, client_id: str, client_secret: str, jwt_key_id: str, private_key: str, private_key_passphrase: str, *, enterprise_id: Optional[str] = None, user_id: Optional[str] = None, algorithm: Optional[JwtAlgorithm] = JwtAlgorithm.RS256, token_storage: TokenStorage = None):
        """
        :param client_id: App client ID
        :type client_id: str
        :param client_secret: App client secret
    ...
    @staticmethod
    def from_config_json_string(config_json_string: str, *, token_storage: Optional[TokenStorage] = None) -> 'JWTConfig':
        """
        ...
        """
        config_json: JwtConfigFile = deserialize(json_to_serialized_data(config_json_string), JwtConfigFile)
        new_config: 'JWTConfig' = JWTConfig(client_id=config_json.box_app_settings.client_id, client_secret=config_json.box_app_settings.client_secret, enterprise_id=config_json.enterprise_id, user_id=config_json.user_id, jwt_key_id=config_json.box_app_settings.app_auth.public_key_id, private_key=config_json.box_app_settings.app_auth.private_key, private_key_passphrase=config_json.box_app_settings.app_auth.passphrase, token_storage=token_storage) if not token_storage == None else JWTConfig(client_id=config_json.box_app_settings.client_id, client_secret=config_json.box_app_settings.client_secret, enterprise_id=config_json.enterprise_id, user_id=config_json.user_id, jwt_key_id=config_json.box_app_settings.app_auth.public_key_id, private_key=config_json.box_app_settings.app_auth.private_key, private_key_passphrase=config_json.box_app_settings.app_auth.passphrase)
        return new_config
    @staticmethod
    def from_config_file(config_file_path: str, *, token_storage: Optional[TokenStorage] = None) -> 'JWTConfig':
        """
        ...
        return JWTConfig.from_config_json_string(config_json_string, token_storage=token_storage)
...
class BoxJWTAuth(Authentication):
    def __init__(self, config: JWTConfig, **kwargs):
        ...
        self.subject_id = self.config.enterprise_id if not self.config.enterprise_id == None else self.config.user_id
        self.subject_type = 'enterprise' if not self.config.enterprise_id == None else 'user'
    def refresh_token(self, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        """
        ...
        """
        if is_browser():
            raise BoxSDKError(message='JWT auth is not supported in browser environment.')
        alg: JwtAlgorithm = self.config.algorithm if not self.config.algorithm == None else JwtAlgorithm.RS256
        claims: Dict = {'exp': get_epoch_time_in_seconds() + 30, 'box_sub_type': self.subject_type}
        jwt_options: JwtSignOptions = JwtSignOptions(algorithm=alg, audience='https://api.box.com/oauth2/token', subject=self.subject_id, issuer=self.config.client_id, jwtid=get_uuid(), keyid=self.config.jwt_key_id)
        jwt_key: JwtKey = JwtKey(key=self.config.private_key, passphrase=self.config.private_key_passphrase)
        assertion: str = create_jwt_assertion(claims, jwt_key, jwt_options)
        auth_manager: AuthorizationManager = AuthorizationManager(network_session=network_session if not network_session == None else NetworkSession())
        token: AccessToken = auth_manager.request_access_token(PostOAuth2TokenGrantTypeField.URN_IETF_PARAMS_OAUTH_GRANT_TYPE_JWT_BEARER, assertion=assertion, client_id=self.config.client_id, client_secret=self.config.client_secret)
        self.token_storage.store(token)
        return token
    def retrieve_token(self, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        """
        ...
        if old_token == None:
            ...
            return new_token
        return old_token
    def retrieve_authorization_header(self, *, network_session: Optional[NetworkSession] = None) -> str:
        token: AccessToken = self.retrieve_token(network_session=network_session)
        return ''.join(['Bearer ', token.access_token])
    def with_user_subject(self, user_id: str, *, token_storage: TokenStorage = None) -> 'BoxJWTAuth':
        """
        ...
        if token_storage is None:
            token_storage = InMemoryTokenStorage()
        new_config: JWTConfig = JWTConfig(client_id=self.config.client_id, client_secret=self.config.client_secret, enterprise_id=None, user_id=user_id, jwt_key_id=self.config.jwt_key_id, private_key=self.config.private_key, private_key_passphrase=self.config.private_key_passphrase, token_storage=token_storage)
        new_auth: 'BoxJWTAuth' = BoxJWTAuth(config=new_config)
        return new_auth
    def with_enterprise_subject(self, enterprise_id: str, *, token_storage: TokenStorage = None) -> 'BoxJWTAuth':
        """
        ...
        if token_storage is None:
            token_storage = InMemoryTokenStorage()
        new_config: JWTConfig = JWTConfig(client_id=self.config.client_id, client_secret=self.config.client_secret, enterprise_id=enterprise_id, user_id=None, jwt_key_id=self.config.jwt_key_id, private_key=self.config.private_key, private_key_passphrase=self.config.private_key_passphrase, token_storage=token_storage)
        new_auth: 'BoxJWTAuth' = BoxJWTAuth(config=new_config)
        return new_auth
    def downscope_token(self, scopes: List[str], *, resource: Optional[str] = None, shared_link: Optional[str] = None, network_session: Optional[NetworkSession] = None) -> AccessToken:
        """
        ...
        :type network_session: Optional[NetworkSession], optional
        """
        token: Optional[AccessToken] = self.retrieve_token(network_session=network_session)
        if token == None:
            raise BoxSDKError(message='No access token is available. Make an API call to retrieve a token before calling this method.')
        auth_manager: AuthorizationManager = AuthorizationManager(network_session=network_session if not network_session == None else NetworkSession())
        downscoped_token: AccessToken = auth_manager.request_access_token(PostOAuth2TokenGrantTypeField.URN_IETF_PARAMS_OAUTH_GRANT_TYPE_TOKEN_EXCHANGE, subject_token=token.access_token, subject_token_type=PostOAuth2TokenSubjectTokenTypeField.URN_IETF_PARAMS_OAUTH_TOKEN_TYPE_ACCESS_TOKEN, resource=resource, scope=' '.join(scopes), box_shared_link=shared_link)
    ...
    def revoke_token(self, *, network_session: Optional[NetworkSession] = None) -> None:
        ...
        if old_token == None:
            return None
        auth_manager: AuthorizationManager = AuthorizationManager(network_session=network_session if not network_session == None else NetworkSession())
        auth_manager.revoke_access_token(client_id=self.config.client_id, client_secret=self.config.client_secret, token=old_token.access_token)
