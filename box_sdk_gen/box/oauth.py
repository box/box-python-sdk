...
class OAuthConfig:
    def __init__(self, client_id: str, client_secret: str, *, token_storage: TokenStorage = None):
...
class GetAuthorizeUrlOptions:
    def __init__(self, *, client_id: Optional[str] = None, redirect_uri: Optional[str] = None, response_type: Optional[str] = None, state: Optional[str] = None, scope: Optional[str] = None):
        """
...
class BoxOAuth(Authentication):
    ...
    def get_authorize_url(self, *, options: GetAuthorizeUrlOptions = None) -> str:
        ...
        params_map: Dict[str, str] = prepare_params({'client_id': options.client_id if not options.client_id == None else self.config.client_id, 'response_type': options.response_type if not options.response_type == None else 'code', 'redirect_uri': options.redirect_uri, 'state': options.state, 'scope': options.scope})
        return ''.join(['https://account.box.com/api/oauth2/authorize?', sd_to_url_params(serialize(params_map))])
    def get_tokens_authorization_code_grant(self, authorization_code: str, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        """
        ...
        token: AccessToken = auth_manager.request_access_token(PostOAuth2TokenGrantTypeField.AUTHORIZATION_CODE, code=authorization_code, client_id=self.config.client_id, client_secret=self.config.client_secret)
    ...
    def retrieve_token(self, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        ...
        if token == None:
            raise BoxSDKError(message='Access and refresh tokens not available. Authenticate before making any API call first.')
        return token
    def refresh_token(self, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        ...
        token_used_for_refresh: Optional[str] = old_token.refresh_token if not old_token == None else None
        auth_manager: AuthorizationManager = AuthorizationManager(network_session=network_session if not network_session == None else NetworkSession())
        token: AccessToken = auth_manager.request_access_token(PostOAuth2TokenGrantTypeField.REFRESH_TOKEN, client_id=self.config.client_id, client_secret=self.config.client_secret, refresh_token=token_used_for_refresh)
        self.token_storage.store(token)
        return token
    ...
    def revoke_token(self, *, network_session: Optional[NetworkSession] = None) -> None:
        ...
        if token == None:
            return None
        auth_manager: AuthorizationManager = AuthorizationManager(network_session=network_session if not network_session == None else NetworkSession())
        auth_manager.revoke_access_token(client_id=self.config.client_id, client_secret=self.config.client_secret, token=token.access_token)
        return None
