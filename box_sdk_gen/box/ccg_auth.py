...
class CCGConfig:
    def __init__(self, client_id: str, client_secret: str, *, enterprise_id: Optional[str] = None, user_id: Optional[str] = None, token_storage: TokenStorage = None):
        """
...
class BoxCCGAuth(Authentication):
    def __init__(self, config: CCGConfig, **kwargs):
        ...
        self.subject_id = self.config.user_id if not self.config.user_id == None else self.config.enterprise_id
        self.subject_type = PostOAuth2TokenBoxSubjectTypeField.USER if not self.config.user_id == None else PostOAuth2TokenBoxSubjectTypeField.ENTERPRISE
    def refresh_token(self, *, network_session: Optional[NetworkSession] = None) -> AccessToken:
        ...
        token: AccessToken = auth_manager.request_access_token(PostOAuth2TokenGrantTypeField.CLIENT_CREDENTIALS, client_id=self.config.client_id, client_secret=self.config.client_secret, box_subject_type=self.subject_type, box_subject_id=self.subject_id)
    ...
    def with_user_subject(self, user_id: str, *, token_storage: TokenStorage = None) -> 'BoxCCGAuth':
        """
        ...
        if token_storage is None:
            token_storage = InMemoryTokenStorage()
        new_config: CCGConfig = CCGConfig(client_id=self.config.client_id, client_secret=self.config.client_secret, enterprise_id=self.config.enterprise_id, user_id=user_id, token_storage=token_storage)
        return BoxCCGAuth(config=new_config)
    def with_enterprise_subject(self, enterprise_id: str, *, token_storage: TokenStorage = None) -> 'BoxCCGAuth':
        """
        ...
        if token_storage is None:
            token_storage = InMemoryTokenStorage()
        new_config: CCGConfig = CCGConfig(client_id=self.config.client_id, client_secret=self.config.client_secret, enterprise_id=enterprise_id, user_id=None, token_storage=token_storage)
        return BoxCCGAuth(config=new_config)
