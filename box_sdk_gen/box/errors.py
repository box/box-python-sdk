...
class BoxSDKError(GeneratedCodeError):
    def __init__(self, message: str, timestamp: Optional[datetime] = None, error: Optional[Exception] = None, **kwargs):
...
class RequestInfo:
    def __init__(self, method: str, url: str, query_params: Dict[str, str], headers: Dict[str, str], body: Optional[str] = None):
        self.method = method
        self.url = url
...
class ResponseInfo:
    def __init__(self, status_code: int, headers: Dict[str, str], body: Dict = None, raw_body: Optional[str] = None, code: Optional[str] = None, context_info: Optional[Dict[str, Any]] = None, request_id: Optional[str] = None, help_url: Optional[str] = None):
    ...
    def print(self, data_sanitizer: DataSanitizer):
        return ''.join((
            ...
            ''.join(['\n\tBody: ', '\n' if self.body else '', pprint.pformat(data_sanitizer.sanitize_body(self.body), indent=8)]),
        ))


class BoxAPIError(BoxSDKError):
    def __init__(self, request_info: RequestInfo, response_info: ResponseInfo, message: str, timestamp: Optional[datetime] = None, error: Optional[str] = None, *, data_sanitizer: DataSanitizer = None, **kwargs):
