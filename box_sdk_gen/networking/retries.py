...
class BoxRetryStrategy(RetryStrategy):
    def __init__(self, *, max_attempts: int = 5, retry_randomization_factor: float = 0.5, retry_base_interval: float = 1, **kwargs):
        super().__init__(**kwargs)
        self.max_attempts = max_attempts
        self.retry_randomization_factor = retry_randomization_factor
        self.retry_base_interval = retry_base_interval
    def should_retry(self, fetch_options: FetchOptions, fetch_response: FetchResponse, attempt_number: int) -> bool:
        is_successful: bool = fetch_response.status >= 200 and fetch_response.status < 400
        retry_after_header: str = fetch_response.headers.get('Retry-After')
        is_accepted_with_retry_after: bool = fetch_response.status == 202 and not retry_after_header == None
        ...
        if fetch_response.status == 401 and not fetch_options.auth == None:
            fetch_options.auth.refresh_token(network_session=fetch_options.network_session)
            return True
        if is_successful:
            return False
        return False
    def retry_after(self, fetch_options: FetchOptions, fetch_response: FetchResponse, attempt_number: int) -> float:
        ...
        randomization: float = random(1 - self.retry_randomization_factor, 1 + self.retry_randomization_factor)
