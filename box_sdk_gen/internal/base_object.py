...
class BaseObject:
    ...
    def from_dict(cls, data: dict):
        ...
        for key, value in data.items():
            ...
            unpacked_attributes[mapping_field_name] = cls._deserialize(key, value, annotation)
    ...
    def to_dict(self) -> dict:
        ...
        for k, v in vars(self).items():
            ...
            if isinstance(v, NullValue):
                ...
                value = {key: value.to_dict() if isinstance(value, BaseObject) else value for key, value in v.items()}
    ...
    def _deserialize(cls, key, value, annotation=None):
        if annotation is None or value is None:
            return value
        if cls.get_origin(annotation) == Optional:
            return cls._deserialize(key, value, cls.get_args(annotation.__args__))
        if cls.get_origin(annotation) == Union:
            union_without_none_type = [arg for arg in cls.get_args(annotation) if arg is not type(None)]
            if len(union_without_none_type) == 1:
                return cls._deserialize(key, value, union_without_none_type[0])

        if cls.get_origin(annotation) == list:
            return cls._deserialize_list(key, value, annotation)
        elif cls.get_origin(annotation) == Union:
    ...
    def _deserialize_list(cls, key, value, annotation: list):
        try:
            list_type = cls.get_args(annotation)[0]
            return [cls._deserialize(key, list_entry, list_type) for list_entry in value]
        except Exception:
            return value
    ...
    def _deserialize_union(cls, key, value, annotation):
        try:
            possible_types = cls.get_args(annotation)
            if value is None:
            ...
            for possible_type in possible_types:
                if isinstance(possible_type, type) \
                        ...
                        and value.get(possible_type._discriminator[0], None) in possible_type._discriminator[1]:
