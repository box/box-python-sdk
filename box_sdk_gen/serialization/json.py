...
def serialize(obj: Union[BaseObject, dict, list]) -> SerializedData:
    ...
    if isinstance(obj, list):
        obj = [element.to_dict() if isinstance(element, BaseObject) else element for element in obj]
    return obj
...
def sanitize_serialized_data(sd: SerializedData, keys_to_sanitize: Dict[str, str]) -> SerializedData:
    ...
    for key, value in sd.items():
        if key.lower() in keys_to_sanitize and isinstance(value, str):
            ...
            sanitized_dictionary[key] = sanitize_serialized_data(value, keys_to_sanitize)
        else:
