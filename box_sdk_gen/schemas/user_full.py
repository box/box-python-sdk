...
class UserFullEnterpriseField(BaseObject):
    ...
    def __init__(self, *, id: Optional[str] = None, type: Optional[UserFullEnterpriseTypeField] = None, name: Optional[str] = None, **kwargs):
        """
...
to the primary email address., defaults to None
        ...
        """
        super().__init__(id=id, created_at=created_at, modified_at=modified_at, language=language, timezone=timezone, space_amount=space_amount, space_used=space_used, max_upload_size=max_upload_size, status=status, job_title=job_title, phone=phone, address=address, avatar_url=avatar_url, notification_email=notification_email, name=name, login=login, type=type, **kwargs)
        self.role = role
