...
class LegalHoldPoliciesManager:
    ...
    def get_legal_hold_policies(self, *, policy_name: Optional[str] = None, fields: Optional[List[str]] = None, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> LegalHoldPolicies:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/legal_hold_policies']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, LegalHoldPolicies)
    def create_legal_hold_policy(self, policy_name: str, *, description: Optional[str] = None, filter_started_at: Optional[DateTime] = None, filter_ended_at: Optional[DateTime] = None, is_ongoing: Optional[bool] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> LegalHoldPolicy:
        """
...
Required if no filter dates are set., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'policy_name': policy_name, 'description': description, 'filter_started_at': filter_started_at, 'filter_ended_at': filter_ended_at, 'is_ongoing': is_ongoing}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "324432"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/legal_hold_policies/', to_string(legal_hold_policy_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, LegalHoldPolicy)
...
Example: "324432"
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'policy_name': policy_name, 'description': description, 'release_notes': release_notes}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
