...
Example: "33243242"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/sign_requests/', to_string(sign_request_id), '/cancel']), method='POST', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
Example: "33243242"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/sign_requests/', to_string(sign_request_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, SignRequest)
    def get_sign_requests(self, *, marker: Optional[str] = None, limit: Optional[int] = None, senders: Optional[List[str]] = None, shared_requests: Optional[bool] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> SignRequests:
        """
...
Read more about [segments and ethical walls](https://support.box.com/hc/en-us/articles/9920431507603-Understanding-Information-Barriers#h_01GFVJEHQA06N7XEZ4GCZ9GFAQ).
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'source_files': source_files, 'signature_color': signature_color, 'signers': signers, 'parent_folder': parent_folder, 'is_document_preparation_needed': is_document_preparation_needed, 'redirect_url': redirect_url, 'declined_redirect_url': declined_redirect_url, 'are_text_signatures_enabled': are_text_signatures_enabled, 'email_subject': email_subject, 'email_message': email_message, 'are_reminders_enabled': are_reminders_enabled, 'name': name, 'prefill_tags': prefill_tags, 'days_valid': days_valid, 'external_id': external_id, 'template_id': template_id, 'external_system_name': external_system_name}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
