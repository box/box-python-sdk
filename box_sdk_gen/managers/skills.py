...
class UpdateBoxSkillCardsOnFileRequestBody(BaseObject):
    def __init__(self, *, op: Optional[UpdateBoxSkillCardsOnFileRequestBodyOpField] = None, path: Optional[str] = None, value: Optional[Union[KeywordSkillCard, TimelineSkillCard, TranscriptSkillCard, StatusSkillCard]] = None, **kwargs):
        """
...
class UpdateAllSkillCardsOnFileFile(BaseObject):
    ...
    def __init__(self, *, type: Optional[UpdateAllSkillCardsOnFileFileTypeField] = None, id: Optional[str] = None, **kwargs):
        """
...
class UpdateAllSkillCardsOnFileFileVersion(BaseObject):
    ...
    def __init__(self, *, type: Optional[UpdateAllSkillCardsOnFileFileVersionTypeField] = None, id: Optional[str] = None, **kwargs):
        """
...
class SkillsManager:
    ...
    def get_box_skill_cards_on_file(self, file_id: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> SkillCardsMetadata:
        """
...
Example: "12345"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/metadata/global/boxSkillsCards']), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, SkillCardsMetadata)
    def create_box_skill_cards_on_file(self, file_id: str, cards: List[Union[KeywordSkillCard, TimelineSkillCard, TranscriptSkillCard, StatusSkillCard]], *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> SkillCardsMetadata:
        """
...
Example: "12345"
        ...
        return deserialize(response.data, SkillCardsMetadata)
    def update_box_skill_cards_on_file(self, file_id: str, request_body: List[UpdateBoxSkillCardsOnFileRequestBody], *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> SkillCardsMetadata:
        """
...
Example: "12345"
        ...
        return deserialize(response.data, SkillCardsMetadata)
    def delete_box_skill_cards_from_file(self, file_id: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/metadata/global/boxSkillsCards']), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
...
state, and leave it out in most other situations., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'status': status, 'metadata': metadata, 'file': file, 'file_version': file_version, 'usage': usage}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/skill_invocations/', to_string(skill_id)]), method='PUT', headers=headers_map, data=serialize(request_body), content_type='application/json', response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None