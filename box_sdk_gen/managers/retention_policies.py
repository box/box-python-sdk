...
class RetentionPoliciesManager:
    ...
    def get_retention_policies(self, *, policy_name: Optional[str] = None, policy_type: Optional[GetRetentionPoliciesPolicyType] = None, created_by_user_id: Optional[str] = None, fields: Optional[List[str]] = None, limit: Optional[int] = None, marker: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicies:
        """
...
used when paginating using marker-based pagination., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/retention_policies']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, RetentionPolicies)
    def create_retention_policy(self, policy_name: str, policy_type: CreateRetentionPolicyPolicyType, disposition_action: CreateRetentionPolicyDispositionAction, *, description: Optional[str] = None, retention_length: Optional[str] = None, retention_type: Optional[CreateRetentionPolicyRetentionType] = None, can_owner_extend_retention: Optional[bool] = None, are_owners_notified: Optional[bool] = None, custom_notification_recipients: Optional[List[UserMini]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicy:
        """
...
the retention policy duration is about to end., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'policy_name': policy_name, 'description': description, 'policy_type': policy_type, 'disposition_action': disposition_action, 'retention_length': retention_length, 'retention_type': retention_type, 'can_owner_extend_retention': can_owner_extend_retention, 'are_owners_notified': are_owners_notified, 'custom_notification_recipients': custom_notification_recipients}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
    ...
    def get_retention_policy_by_id(self, retention_policy_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicy:
        """
...
Example: "982312"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/retention_policies/', to_string(retention_policy_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None