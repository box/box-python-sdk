...
class CreateTaskItem(BaseObject):
    ...
    def __init__(self, *, id: Optional[str] = None, type: Optional[CreateTaskItemTypeField] = None, **kwargs):
        """
...
Example: "12345"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/tasks']), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Tasks)
    def create_task(self, item: CreateTaskItem, *, action: Optional[CreateTaskAction] = None, message: Optional[str] = None, due_at: Optional[DateTime] = None, completion_rule: Optional[CreateTaskCompletionRule] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Task:
        """
...
approve the the task in order for it to be considered completed., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'item': item, 'action': action, 'message': message, 'due_at': due_at, 'completion_rule': completion_rule}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/tasks/', to_string(task_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Task)
    def update_task_by_id(self, task_id: str, *, action: Optional[UpdateTaskByIdAction] = None, message: Optional[str] = None, due_at: Optional[DateTime] = None, completion_rule: Optional[UpdateTaskByIdCompletionRule] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Task:
        """
...
approve the the task in order for it to be considered completed., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'action': action, 'message': message, 'due_at': due_at, 'completion_rule': completion_rule}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/tasks/', to_string(task_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None