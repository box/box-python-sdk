...
class CreateMetadataTemplateFields(BaseObject):
    ...
    def __init__(self, type: CreateMetadataTemplateFieldsTypeField, key: str, display_name: str, *, description: Optional[str] = None, hidden: Optional[bool] = None, options: Optional[List[CreateMetadataTemplateFieldsOptionsField]] = None, **kwargs):
        """
...
class MetadataTemplatesManager:
    ...
    def get_metadata_templates_by_instance_id(self, metadata_instance_id: str, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplates:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        query_params_map: Dict[str, str] = prepare_params({'metadata_instance_id': to_string(metadata_instance_id), 'marker': to_string(marker), 'limit': to_string(limit)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplates)
    def get_metadata_template(self, scope: GetMetadataTemplateScope, template_key: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplate:
        """
...
Example: "properties"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/', to_string(scope), '/', to_string(template_key), '/schema']), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplate)
    def update_metadata_template(self, scope: UpdateMetadataTemplateScope, template_key: str, request_body: List[UpdateMetadataTemplateRequestBody], *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplate:
        """
...
Example: "properties"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/', to_string(scope), '/', to_string(template_key), '/schema']), method='PUT', headers=headers_map, data=serialize(request_body), content_type='application/json-patch+json', response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplate)
    def delete_metadata_template(self, scope: DeleteMetadataTemplateScope, template_key: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
Example: "properties"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/', to_string(scope), '/', to_string(template_key), '/schema']), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
...
Example: "f7a9891f"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/', to_string(template_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplate)
    def get_global_metadata_templates(self, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplates:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/global']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplates)
    def get_enterprise_metadata_templates(self, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplates:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        query_params_map: Dict[str, str] = prepare_params({'marker': to_string(marker), 'limit': to_string(limit)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_templates/enterprise']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataTemplates)
    def create_metadata_template(self, scope: str, display_name: str, *, template_key: Optional[str] = None, hidden: Optional[bool] = None, fields: Optional[List[CreateMetadataTemplateFields]] = None, copy_instance_on_item_copy: Optional[bool] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataTemplate:
        """
...
file or folder when it is copied., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'scope': scope, 'templateKey': template_key, 'displayName': display_name, 'hidden': hidden, 'fields': fields, 'copyInstanceOnItemCopy': copy_instance_on_item_copy}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
