...
always represented by the ID `0`.
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/folder_locks']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, FolderLocks)
    def create_folder_lock(self, folder: CreateFolderLockFolder, *, locked_operations: Optional[CreateFolderLockLockedOperations] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FolderLock:
        """
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/folder_locks/', to_string(folder_lock_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None