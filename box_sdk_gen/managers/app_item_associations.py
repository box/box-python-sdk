...
class AppItemAssociationsManager:
    ...
    def get_file_app_item_associations(self, file_id: str, *, limit: Optional[int] = None, marker: Optional[str] = None, application_type: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AppItemAssociations:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        query_params_map: Dict[str, str] = prepare_params({'limit': to_string(limit), 'marker': to_string(marker), 'application_type': to_string(application_type)})
    ...
    def get_folder_app_item_associations(self, folder_id: str, *, limit: Optional[int] = None, marker: Optional[str] = None, application_type: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AppItemAssociations:
        """
        **This is a beta feature, which means that its availability might be limited.**
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        query_params_map: Dict[str, str] = prepare_params({'limit': to_string(limit), 'marker': to_string(marker), 'application_type': to_string(application_type)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/folders/', to_string(folder_id), '/app_item_associations']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, AppItemAssociations)