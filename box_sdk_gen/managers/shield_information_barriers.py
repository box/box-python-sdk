...
Example: "1910967"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barriers/', to_string(shield_information_barrier_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, ShieldInformationBarrier)
    def update_shield_information_barrier_status(self, id: str, status: UpdateShieldInformationBarrierStatusStatus, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrier:
        """
        ...
        return deserialize(response.data, ShieldInformationBarrier)
    def get_shield_information_barriers(self, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarriers:
        """
...
used when paginating using marker-based pagination., defaults to None
    ...
    def create_shield_information_barrier(self, enterprise: EnterpriseBase, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrier:
        """
