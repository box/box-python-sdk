...
class FileVersionsManager:
    ...
    def get_file_versions(self, file_id: str, *, fields: Optional[List[str]] = None, limit: Optional[int] = None, offset: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FileVersions:
        """
...
with a 400 response., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/versions']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, FileVersions)
    def get_file_version_by_id(self, file_id: str, file_version_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FileVersionFull:
        """
...
to the fields requested., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/versions/', to_string(file_version_id)]), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
has changed since., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/files/', to_string(file_id), '/versions/', to_string(file_version_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def update_file_version_by_id(self, file_id: str, file_version_id: str, *, trashed_at: Union[Optional[str], NullValue] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FileVersionFull:
        """
...
the date and restore the file., defaults to None
        ...
        return deserialize(response.data, FileVersionFull)
    def promote_file_version(self, file_id: str, *, id: Optional[str] = None, type: Optional[PromoteFileVersionType] = None, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FileVersionFull:
        """
