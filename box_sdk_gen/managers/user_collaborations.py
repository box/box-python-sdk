...
class CreateCollaborationItem(BaseObject):
    ...
    def __init__(self, *, type: Optional[CreateCollaborationItemTypeField] = None, id: Optional[str] = None, **kwargs):
        """
...
class CreateCollaborationAccessibleBy(BaseObject):
    ...
    def __init__(self, type: CreateCollaborationAccessibleByTypeField, *, id: Optional[str] = None, login: Optional[str] = None, **kwargs):
        """
...
class UserCollaborationsManager:
    ...
    def get_collaboration_by_id(self, collaboration_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Collaboration:
        """
...
to the fields requested., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/collaborations/', to_string(collaboration_id)]), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Collaboration)
    def update_collaboration_by_id(self, collaboration_id: str, role: UpdateCollaborationByIdRole, *, status: Optional[UpdateCollaborationByIdStatus] = None, expires_at: Optional[DateTime] = None, can_view_path: Optional[bool] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Optional[Collaboration]:
        """
...
`can_view_path` can only be used for folder collaborations., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'role': role, 'status': status, 'expires_at': expires_at, 'can_view_path': can_view_path}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "1234"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/collaborations/', to_string(collaboration_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def create_collaboration(self, item: CreateCollaborationItem, accessible_by: CreateCollaborationAccessibleBy, role: CreateCollaborationRole, *, is_access_only: Optional[bool] = None, can_view_path: Optional[bool] = None, expires_at: Optional[DateTime] = None, fields: Optional[List[str]] = None, notify: Optional[bool] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Collaboration:
        """
...
for the action performed., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'item': item, 'accessible_by': accessible_by, 'role': role, 'is_access_only': is_access_only, 'can_view_path': can_view_path, 'expires_at': expires_at}
