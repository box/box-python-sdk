...
class CreateRetentionPolicyAssignmentAssignTo(BaseObject):
    ...
    def __init__(self, type: CreateRetentionPolicyAssignmentAssignToTypeField, *, id: Optional[str] = None, **kwargs):
        """
...
class RetentionPolicyAssignmentsManager:
    ...
    def get_retention_policy_assignments(self, retention_policy_id: str, *, type: Optional[GetRetentionPolicyAssignmentsType] = None, fields: Optional[List[str]] = None, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicyAssignments:
        """
...
used when paginating using marker-based pagination., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/retention_policies/', to_string(retention_policy_id), '/assignments']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, RetentionPolicyAssignments)
    def create_retention_policy_assignment(self, policy_id: str, assign_to: CreateRetentionPolicyAssignmentAssignTo, *, filter_fields: Optional[List[CreateRetentionPolicyAssignmentFilterFields]] = None, start_date_field: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicyAssignment:
        """
...
this field can be a date field's metadata attribute key id., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'policy_id': policy_id, 'assign_to': assign_to, 'filter_fields': filter_fields, 'start_date_field': start_date_field}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
    ...
    def get_retention_policy_assignment_by_id(self, retention_policy_assignment_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> RetentionPolicyAssignment:
        """
...
Example: "1233123"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/retention_policy_assignments/', to_string(retention_policy_assignment_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def get_files_under_retention_policy_assignment(self, retention_policy_assignment_id: str, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FilesUnderRetention:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/retention_policy_assignments/', to_string(retention_policy_assignment_id), '/files_under_retention']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
