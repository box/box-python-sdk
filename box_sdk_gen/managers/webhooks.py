...
class CreateWebhookTarget(BaseObject):
    ...
    def __init__(self, *, id: Optional[str] = None, type: Optional[CreateWebhookTargetTypeField] = None, **kwargs):
        """
...
class UpdateWebhookByIdTarget(BaseObject):
    _discriminator = 'type', {'file', 'folder'}
    def __init__(self, *, id: Optional[str] = None, type: Optional[UpdateWebhookByIdTargetTypeField] = None, **kwargs):
        """
        :param id: The ID of the item to trigger a webhook, defaults to None
        :type id: Optional[str], optional
        :param type: The type of item to trigger a webhook, defaults to None
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/webhooks']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Webhooks)
    def create_webhook(self, target: CreateWebhookTarget, address: str, triggers: List[CreateWebhookTriggers], *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Webhook:
        """
        Creates a webhook.
...
to be triggered for
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'target': target, 'address': address, 'triggers': triggers}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "3321123"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/webhooks/', to_string(webhook_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Webhook)
    def update_webhook_by_id(self, webhook_id: str, *, target: Optional[UpdateWebhookByIdTarget] = None, address: Optional[str] = None, triggers: Optional[List[UpdateWebhookByIdTriggers]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Webhook:
        """
        Updates a webhook.
        :param webhook_id: The ID of the webhook.
Example: "3321123"
...
Example: "3321123"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/webhooks/', to_string(webhook_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    @staticmethod
    def validate_message(body: str, headers: Dict[str, str], primary_key: str, *, secondary_key: Optional[str] = None, max_age: Optional[int] = 600) -> bool:
        """
        ...
        if current_epoch - max_age > date_time_to_epoch_seconds(delivery_timestamp) or date_time_to_epoch_seconds(delivery_timestamp) > current_epoch:
            return False
        if primary_key and compute_webhook_signature(body, headers, primary_key) == headers.get('box-signature-primary'):
            return True
        if secondary_key and compute_webhook_signature(body, headers, secondary_key) == headers.get('box-signature-secondary'):
            return True
        return False