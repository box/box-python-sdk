...
class CreateLegalHoldPolicyAssignmentAssignTo(BaseObject):
    _discriminator = 'type', {'file', 'file_version', 'folder', 'user', 'ownership', 'interaction'}
    def __init__(self, type: CreateLegalHoldPolicyAssignmentAssignToTypeField, id: str, **kwargs):
        """
...
class LegalHoldPolicyAssignmentsManager:
    ...
    def get_legal_hold_policy_assignments(self, policy_id: str, *, assign_to_type: Optional[GetLegalHoldPolicyAssignmentsAssignToType] = None, assign_to_id: Optional[str] = None, marker: Optional[str] = None, limit: Optional[int] = None, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> LegalHoldPolicyAssignments:
        """
...
to the fields requested., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/legal_hold_policy_assignments']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, LegalHoldPolicyAssignments)
    def create_legal_hold_policy_assignment(self, policy_id: str, assign_to: CreateLegalHoldPolicyAssignmentAssignTo, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> LegalHoldPolicyAssignment:
        """
...
Example: "753465"
        ...
        return None
    def get_legal_hold_policy_assignment_file_on_hold(self, legal_hold_policy_assignment_id: str, *, marker: Optional[str] = None, limit: Optional[int] = None, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> FilesOnHold:
        """
...
to the fields requested., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/legal_hold_policy_assignments/', to_string(legal_hold_policy_assignment_id), '/files_on_hold']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
