...
with a 400 response., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/users/', to_string(user_id), '/memberships']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
with a 400 response., defaults to None
        ...
        query_params_map: Dict[str, str] = prepare_params({'limit': to_string(limit), 'offset': to_string(offset)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/groups/', to_string(group_id), '/memberships']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, GroupMemberships)
    def create_group_membership(self, user: CreateGroupMembershipUser, group: CreateGroupMembershipGroup, *, role: Optional[CreateGroupMembershipRole] = None, configurable_permissions: Union[Optional[Dict[str, bool]], NullValue] = None, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> GroupMembership:
        """
...
to the fields requested., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'user': user, 'group': group, 'role': role, 'configurable_permissions': configurable_permissions}
    ...
    def get_group_membership_by_id(self, group_membership_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> GroupMembership:
        """
...
to the fields requested., defaults to None
        ...
        return deserialize(response.data, GroupMembership)
    def update_group_membership_by_id(self, group_membership_id: str, *, role: Optional[UpdateGroupMembershipByIdRole] = None, configurable_permissions: Union[Optional[Dict[str, bool]], NullValue] = None, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> GroupMembership:
        """
...
to the fields requested., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'role': role, 'configurable_permissions': configurable_permissions}
        query_params_map: Dict[str, str] = prepare_params({'fields': to_string(fields)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
