...
class TermsOfServicesManager:
    ...
    def get_terms_of_service(self, *, tos_type: Optional[GetTermsOfServiceTosType] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TermsOfServices:
        """
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/terms_of_services']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, TermsOfServices)
    def create_terms_of_service(self, status: CreateTermsOfServiceStatus, text: str, *, tos_type: Optional[CreateTermsOfServiceTosType] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TermsOfService:
        """
...
service for., defaults to None
    ...
    def get_terms_of_service_by_id(self, terms_of_service_id: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TermsOfService:
        """
...
Example: "324234"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/terms_of_services/', to_string(terms_of_service_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, TermsOfService)
    def update_terms_of_service_by_id(self, terms_of_service_id: str, status: UpdateTermsOfServiceByIdStatus, text: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TermsOfService:
        """
