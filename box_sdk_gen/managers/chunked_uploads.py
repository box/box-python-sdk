...
class _PartAccumulator:
    def __init__(self, last_index: int, parts: List[UploadPart], file_size: int, upload_part_url: str, file_hash: Hash):
...
class ChunkedUploadsManager:
    def __init__(self, *, auth: Optional[Authentication] = None, network_session: NetworkSession = None):
...
Example: "12345"
        ...
        return deserialize(response.data, UploadSession)
    def get_file_upload_session_by_url(self, url: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> UploadSession:
        """
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=url, method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, UploadSession)
...
Example: "D5E3F7A"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.upload_url, '/2.0/files/upload_sessions/', to_string(upload_session_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, UploadSession)
    def upload_file_part_by_url(self, url: str, request_body: ByteStream, digest: str, content_range: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> UploadedPart:
        """
...
* The higher bound must be a multiple of the part size - 1.
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=url, method='PUT', headers=headers_map, file_stream=request_body, content_type='application/octet-stream', response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
* The higher bound must be a multiple of the part size - 1.
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({'digest': to_string(digest), 'content-range': to_string(content_range), **extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.upload_url, '/2.0/files/upload_sessions/', to_string(upload_session_id)]), method='PUT', headers=headers_map, file_stream=request_body, content_type='application/octet-stream', response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, UploadedPart)
    def delete_file_upload_session_by_url(self, url: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=url, method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
...
Example: "D5E3F7A"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.upload_url, '/2.0/files/upload_sessions/', to_string(upload_session_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def get_file_upload_session_parts_by_url(self, url: str, *, offset: Optional[int] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> UploadParts:
        """
...
with a 400 response., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=url, method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
with a 400 response., defaults to None
        ...
        query_params_map: Dict[str, str] = prepare_params({'offset': to_string(offset), 'limit': to_string(limit)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.upload_url, '/2.0/files/upload_sessions/', to_string(upload_session_id), '/parts']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, UploadParts)
    def create_file_upload_session_commit_by_url(self, url: str, parts: List[UploadPart], digest: str, *, if_match: Optional[str] = None, if_none_match: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Optional[Files]:
        """
...
changed since., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=url, method='POST', headers=headers_map, data=serialize(request_body), content_type='application/json', response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        if to_string(response.status) == '202':
            return None
        return deserialize(response.data, Files)
    def create_file_upload_session_commit(self, upload_session_id: str, parts: List[UploadPart], digest: str, *, if_match: Optional[str] = None, if_none_match: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Optional[Files]:
        """
        Close an upload session and create a file from the uploaded chunks.
...
changed since., defaults to None
    ...
    def _reducer(self, acc: _PartAccumulator, chunk: ByteStream) -> _PartAccumulator:
        ...
        content_range: str = ''.join(['bytes ', to_string(bytes_start), '-', to_string(bytes_end), '/', to_string(acc.file_size)])
        uploaded_part: UploadedPart = self.upload_file_part_by_url(acc.upload_part_url, generate_byte_stream_from_buffer(chunk_buffer), digest, content_range)
        ...
        return _PartAccumulator(last_index=bytes_end, parts=parts + [part], file_size=acc.file_size, upload_part_url=acc.upload_part_url, file_hash=acc.file_hash)
    def upload_big_file(self, file: ByteStream, file_name: str, file_size: int, parent_folder_id: str) -> FileFull:
        """
        ...
        """
        upload_session: UploadSession = self.create_file_upload_session(parent_folder_id, file_size, file_name)
        ...
        results: _PartAccumulator = reduce_iterator(chunks_iterator, self._reducer, _PartAccumulator(last_index=-1, parts=[], file_size=file_size, upload_part_url=upload_part_url, file_hash=file_hash))
        parts: List[UploadPart] = results.parts
        processed_session_parts: UploadParts = self.get_file_upload_session_parts_by_url(list_parts_url)
        assert processed_session_parts.total_count == total_parts
        sha_1: str = file_hash.digest_hash('base64')
        digest: str = ''.join(['sha=', sha_1])
        committed_session: Optional[Files] = self.create_file_upload_session_commit_by_url(commit_url, parts, digest)
