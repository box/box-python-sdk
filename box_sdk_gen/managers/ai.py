...
class CreateAiTextGenItems(BaseObject):
    ...
    def __init__(self, id: str, *, type: CreateAiTextGenItemsTypeField = CreateAiTextGenItemsTypeField.FILE, content: Optional[str] = None, **kwargs):
        """
...
class CreateAiExtractStructuredMetadataTemplate(BaseObject):
    ...
    def __init__(self, *, template_key: Optional[str] = None, type: Optional[CreateAiExtractStructuredMetadataTemplateTypeField] = None, scope: Optional[str] = None, **kwargs):
        """
...
class CreateAiExtractStructuredFields(BaseObject):
    ...
    def __init__(self, key: str, *, description: Optional[str] = None, display_name: Optional[str] = None, prompt: Optional[str] = None, type: Optional[str] = None, options: Optional[List[CreateAiExtractStructuredFieldsOptionsField]] = None, **kwargs):
        """
        :param key: A unique identifier for the field.
        :type key: str
...
class AiManager:
    ...
    def create_ai_ask(self, mode: CreateAiAskMode, prompt: str, items: List[AiItemAsk], *, dialogue_history: Optional[List[AiDialogueHistory]] = None, include_citations: Optional[bool] = None, ai_agent: Optional[Union[AiAgentAsk, AiAgentReference]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Optional[AiResponseFull]:
        """
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'mode': mode, 'prompt': prompt, 'items': items, 'dialogue_history': dialogue_history, 'include_citations': include_citations, 'ai_agent': ai_agent}
    ...
    def create_ai_text_gen(self, prompt: str, items: List[CreateAiTextGenItems], *, dialogue_history: Optional[List[AiDialogueHistory]] = None, ai_agent: Optional[Union[AiAgentReference, AiAgentTextGen]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AiResponse:
        """
...
If the file size exceeds 1MB, the first 1MB of text representation will be processed.
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'prompt': prompt, 'items': items, 'dialogue_history': dialogue_history, 'ai_agent': ai_agent}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
    ...
    def get_ai_agent_default_config(self, mode: GetAiAgentDefaultConfigMode, *, language: Optional[str] = None, model: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Union[AiAgentAsk, AiAgentTextGen, AiAgentExtract, AiAgentExtractStructured]:
        """
...
If the language is not supported the default agent config is returned., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        query_params_map: Dict[str, str] = prepare_params({'mode': to_string(mode), 'language': to_string(language), 'model': to_string(model)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/ai_agent_default']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Union[AiAgentAsk, AiAgentTextGen, AiAgentExtract, AiAgentExtractStructured])
    def create_ai_extract(self, prompt: str, items: List[AiItemBase], *, ai_agent: Optional[Union[AiAgentReference, AiAgentExtract]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AiResponse:
        """
...
For your request to work, you must provide either `metadata_template` or `fields`, but not both., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'items': items, 'metadata_template': metadata_template, 'fields': fields, 'ai_agent': ai_agent}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
