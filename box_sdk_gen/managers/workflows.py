...
class StartWorkflowFiles(BaseObject):
    ...
    def __init__(self, *, type: Optional[StartWorkflowFilesTypeField] = None, id: Optional[str] = None, **kwargs):
        """
...
class StartWorkflowFolder(BaseObject):
    ...
    def __init__(self, *, type: Optional[StartWorkflowFolderTypeField] = None, id: Optional[str] = None, **kwargs):
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/workflows']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Workflows)
    def start_workflow(self, workflow_id: str, flow: StartWorkflowFlow, files: List[StartWorkflowFiles], folder: StartWorkflowFolder, *, type: Optional[StartWorkflowType] = None, outcomes: Optional[List[Outcome]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
must be in the workflow's configured folder.
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'type': type, 'flow': flow, 'files': files, 'folder': folder, 'outcomes': outcomes}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
