...
class SearchByMetadataQueryOrderBy(BaseObject):
    def __init__(self, *, field_key: Optional[str] = None, direction: Optional[SearchByMetadataQueryOrderByDirectionField] = None, **kwargs):
        """
...
`scope` and `templateKey` can be defined., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'from': from_, 'query': query, 'query_params': query_params, 'ancestor_folder_id': ancestor_folder_id, 'order_by': order_by, 'limit': limit, 'marker': marker, 'fields': fields}
...
Data available from 2023-02-01 onwards., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/search']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Union[SearchResults, SearchResultsWithSharedLinks])