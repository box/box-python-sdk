...
from box_sdk_gen.schemas.post_o_auth_2_token_refresh_access_token import PostOAuth2TokenRefreshAccessToken
...
class AuthorizationManager:
    ...
    def authorize_user(self, response_type: AuthorizeUserResponseType, client_id: str, *, redirect_uri: Optional[str] = None, state: Optional[str] = None, scope: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
that the token should be generated for., defaults to None
    ...
    def refresh_access_token(self, client_id: str, client_secret: str, refresh_token: str, *, grant_type: RefreshAccessTokenGrantType = RefreshAccessTokenGrantType.REFRESH_TOKEN, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AccessToken:
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'grant_type': grant_type, 'client_id': client_id, 'client_secret': client_secret, 'refresh_token': refresh_token}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/oauth2/token#refresh']), method='POST', headers=headers_map, data=serialize(request_body), content_type='application/x-www-form-urlencoded', response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, AccessToken)
    def revoke_access_token(self, *, client_id: Optional[str] = None, client_secret: Optional[str] = None, token: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
an access token., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'client_id': client_id, 'client_secret': client_secret, 'token': token}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/oauth2/revoke']), method='POST', headers=headers_map, data=serialize(request_body), content_type='application/x-www-form-urlencoded', response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None