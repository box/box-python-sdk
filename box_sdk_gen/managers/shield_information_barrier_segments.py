...
Example: "3423"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barrier_segments/', to_string(shield_information_barrier_segment_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
Example: "3423"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barrier_segments/', to_string(shield_information_barrier_segment_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def update_shield_information_barrier_segment_by_id(self, shield_information_barrier_segment_id: str, *, name: Optional[str] = None, description: Union[Optional[str], NullValue] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrierSegment:
        """
...
the shield information barrier segment., defaults to None
        ...
        return deserialize(response.data, ShieldInformationBarrierSegment)
    def get_shield_information_barrier_segments(self, shield_information_barrier_id: str, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrierSegments:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barrier_segments']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
    ...
    def create_shield_information_barrier_segment(self, shield_information_barrier: ShieldInformationBarrierBase, name: str, *, description: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrierSegment:
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'shield_information_barrier': shield_information_barrier, 'name': name, 'description': description}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
