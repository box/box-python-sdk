...
class CreateShieldInformationBarrierSegmentRestrictionShieldInformationBarrierSegmentTypeField(str, Enum):
    SHIELD_INFORMATION_BARRIER_SEGMENT = 'shield_information_barrier_segment'

class CreateShieldInformationBarrierSegmentRestrictionShieldInformationBarrierSegment(BaseObject):
    _discriminator = 'type', {'shield_information_barrier_segment'}
    def __init__(self, *, id: Optional[str] = None, type: Optional[CreateShieldInformationBarrierSegmentRestrictionShieldInformationBarrierSegmentTypeField] = None, **kwargs):
        """
...
class CreateShieldInformationBarrierSegmentRestrictionRestrictedSegmentTypeField(str, Enum):
    SHIELD_INFORMATION_BARRIER_SEGMENT = 'shield_information_barrier_segment'

class CreateShieldInformationBarrierSegmentRestrictionRestrictedSegment(BaseObject):
    _discriminator = 'type', {'shield_information_barrier_segment'}
    def __init__(self, *, id: Optional[str] = None, type: Optional[CreateShieldInformationBarrierSegmentRestrictionRestrictedSegmentTypeField] = None, **kwargs):
        """
...
Example: "4563"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barrier_segment_restrictions/', to_string(shield_information_barrier_segment_restriction_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
Example: "4563"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/shield_information_barrier_segment_restrictions/', to_string(shield_information_barrier_segment_restriction_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def get_shield_information_barrier_segment_restrictions(self, shield_information_barrier_segment_id: str, *, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ShieldInformationBarrierSegmentRestrictions:
        """
...
restriction for this member., defaults to CreateShieldInformationBarrierSegmentRestrictionType.SHIELD_INFORMATION_BARRIER_SEGMENT_RESTRICTION
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'type': type, 'shield_information_barrier': shield_information_barrier, 'shield_information_barrier_segment': shield_information_barrier_segment, 'restricted_segment': restricted_segment}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
