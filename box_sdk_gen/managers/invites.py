...
class InvitesManager:
    ...
    def create_invite(self, enterprise: CreateInviteEnterprise, actionable_by: CreateInviteActionableBy, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Invite:
        """
...
to the fields requested., defaults to None
    ...
    def get_invite_by_id(self, invite_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Invite:
        """
...
to the fields requested., defaults to None
        ...
        query_params_map: Dict[str, str] = prepare_params({'fields': to_string(fields)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/invites/', to_string(invite_id)]), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, Invite)