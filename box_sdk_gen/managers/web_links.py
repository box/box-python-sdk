...
class UpdateWebLinkByIdSharedLink(BaseObject):
    def __init__(self, *, access: Optional[UpdateWebLinkByIdSharedLinkAccessField] = None, password: Optional[str] = None, vanity_name: Optional[str] = None, unshared_at: Optional[DateTime] = None, **kwargs):
        """
...
class WebLinksManager:
    ...
    def create_web_link(self, url: str, parent: CreateWebLinkParent, *, name: Optional[str] = None, description: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> WebLink:
        """
...
`"http://"` or `"https://"`.
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'url': url, 'parent': parent, 'name': name, 'description': description}
...
or folders nested within the item., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/web_links/', to_string(web_link_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, WebLink)
    def update_web_link_by_id(self, web_link_id: str, *, url: Optional[str] = None, parent: Optional[UpdateWebLinkByIdParent] = None, name: Optional[str] = None, description: Optional[str] = None, shared_link: Optional[UpdateWebLinkByIdSharedLink] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> WebLink:
        """
...
`"http://"` or `"https://"`., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'url': url, 'parent': parent, 'name': name, 'description': description, 'shared_link': shared_link}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/web_links/', to_string(web_link_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None