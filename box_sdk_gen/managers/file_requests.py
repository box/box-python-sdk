...
class CreateFileRequestCopyFolder(BaseObject):
    ...
    def __init__(self, id: str, *, type: Optional[CreateFileRequestCopyFolderTypeField] = None, **kwargs):
        """
...
Example: "123"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/file_requests/', to_string(file_request_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
...
has changed since., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'title': title, 'description': description, 'status': status, 'is_email_required': is_email_required, 'is_description_required': is_description_required, 'expires_at': expires_at}
...
Example: "123"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/file_requests/', to_string(file_request_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
...
This will default to the value on the existing file request., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'folder': folder, 'title': title, 'description': description, 'status': status, 'is_email_required': is_email_required, 'is_description_required': is_description_required, 'expires_at': expires_at}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
