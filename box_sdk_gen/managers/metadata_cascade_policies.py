...
with a 400 response., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_cascade_policies']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataCascadePolicies)
    def create_metadata_cascade_policy(self, folder_id: str, scope: CreateMetadataCascadePolicyScope, template_key: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> MetadataCascadePolicy:
        """
...
[folder]: e://get-folders-id-metadata
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'folder_id': folder_id, 'scope': scope, 'templateKey': template_key}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "6fd4ff89-8fc1-42cf-8b29-1890dedd26d7"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_cascade_policies/', to_string(metadata_cascade_policy_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, MetadataCascadePolicy)
...
Example: "6fd4ff89-8fc1-42cf-8b29-1890dedd26d7"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/metadata_cascade_policies/', to_string(metadata_cascade_policy_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None
    def apply_metadata_cascade_policy(self, metadata_cascade_policy_id: str, conflict_resolution: ApplyMetadataCascadePolicyConflictResolution, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
