...
class ZipDownloadsManager:
    ...
    def create_zip_download(self, items: List[CreateZipDownloadItems], *, download_file_name: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ZipDownload:
        """
...
`.zip` file extension, for example `January Financials.zip`., defaults to None
    ...
    def get_zip_download_content(self, download_url: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ByteStream:
        """
 ...
 Example: `https://dl.boxcloud.com/2.0/zip_downloads/29l00nfxDyHOt7RphI9zT_w==nDnZEDjY2S8iEWWCHEEiptFxwoWojjlibZjJ6geuE5xnXENDTPxzgbks_yY=/content`
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=download_url, method='GET', headers=headers_map, response_format=ResponseFormat.BINARY, auth=self.auth, network_session=self.network_session))
        return response.content
    def get_zip_download_status(self, status_url: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ZipDownloadStatus:
        """
 ...
 Example: `https://dl.boxcloud.com/2.0/zip_downloads/29l00nfxDyHOt7RphI9zT_w==nDnZEDjY2S8iEWWCHEEiptFxwoWojjlibZjJ6geuE5xnXENDTPxzgbks_yY=/status`
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=status_url, method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, ZipDownloadStatus)
    def download_zip(self, items: List[DownloadZipItems], *, download_file_name: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> ByteStream:
        """
...
`.zip` file extension, for example `January Financials.zip`., defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'items': items, 'download_file_name': download_file_name}
        zip_download_session: ZipDownload = self.create_zip_download(items, download_file_name=download_file_name, extra_headers=extra_headers)
        return self.get_zip_download_content(zip_download_session.download_url, extra_headers=extra_headers)