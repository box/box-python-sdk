...
class CreateTaskAssignmentTask(BaseObject):
    ...
    def __init__(self, id: str, *, type: CreateTaskAssignmentTaskTypeField = CreateTaskAssignmentTaskTypeField.TASK, **kwargs):
        """
...
Example: "12345"
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/tasks/', to_string(task_id), '/assignments']), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, TaskAssignments)
    def create_task_assignment(self, task: CreateTaskAssignmentTask, assign_to: CreateTaskAssignmentAssignTo, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TaskAssignment:
        """
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/task_assignments/', to_string(task_assignment_id)]), method='GET', headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, TaskAssignment)
    def update_task_assignment_by_id(self, task_assignment_id: str, *, message: Optional[str] = None, resolution_state: Optional[UpdateTaskAssignmentByIdResolutionState] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> TaskAssignment:
        """
...
Example: "12345"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/task_assignments/', to_string(task_assignment_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None