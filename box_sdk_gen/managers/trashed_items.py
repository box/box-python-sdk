...
class TrashedItemsManager:
    ...
    def get_trashed_items(self, *, fields: Optional[List[str]] = None, limit: Optional[int] = None, offset: Optional[int] = None, usemarker: Optional[bool] = None, marker: Optional[str] = None, direction: Optional[GetTrashedItemsDirection] = None, sort: Optional[GetTrashedItemsSort] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> Items:
        """
...
This parameter is not supported when using marker-based pagination., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/folders/trash/items']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
