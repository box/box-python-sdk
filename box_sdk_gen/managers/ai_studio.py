...
class AiStudioManager:
    ...
    def get_ai_agents(self, *, mode: Optional[List[str]] = None, fields: Optional[List[str]] = None, agent_state: Optional[List[str]] = None, include_box_default: Optional[bool] = None, marker: Optional[str] = None, limit: Optional[int] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AiMultipleAgentResponse:
        """
        ...
        if extra_headers is None:
            extra_headers = {}
        query_params_map: Dict[str, str] = prepare_params({'mode': to_string(mode), 'fields': to_string(fields), 'agent_state': to_string(agent_state), 'include_box_default': to_string(include_box_default), 'marker': to_string(marker), 'limit': to_string(limit)})
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/ai_agents']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, AiMultipleAgentResponse)
    def create_ai_agent(self, name: str, access_state: str, *, type: CreateAiAgentType = CreateAiAgentType.AI_AGENT, icon_reference: Optional[str] = None, allowed_entities: Optional[List[AiAgentAllowedEntity]] = None, ask: Optional[AiStudioAgentAsk] = None, text_gen: Optional[AiStudioAgentTextGen] = None, extract: Optional[AiStudioAgentExtract] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AiSingleAgentResponseFull:
        """
...
where possible values of `file_name` are: `logo_boxAi.png`,`logo_stamp.png`,`logo_legal.png`,`logo_finance.png`,`logo_config.png`,`logo_handshake.png`,`logo_analytics.png`,`logo_classification.png`, defaults to None
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'type': type, 'name': name, 'access_state': access_state, 'icon_reference': icon_reference, 'allowed_entities': allowed_entities, 'ask': ask, 'text_gen': text_gen, 'extract': extract}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
where possible values of `file_name` are: `logo_boxAi.png`,`logo_stamp.png`,`logo_legal.png`,`logo_finance.png`,`logo_config.png`,`logo_handshake.png`,`logo_analytics.png`,`logo_classification.png`, defaults to None
        ...
        return deserialize(response.data, AiSingleAgentResponseFull)
    def get_ai_agent_by_id(self, agent_id: str, *, fields: Optional[List[str]] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> AiSingleAgentResponseFull:
        """
...
Example: "1234"
        ...
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/ai_agents/', to_string(agent_id)]), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, AiSingleAgentResponseFull)
    def delete_ai_agent_by_id(self, agent_id: str, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> None:
        """
...
Example: "1234"
        ...
        if extra_headers is None:
            extra_headers = {}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/ai_agents/', to_string(agent_id)]), method='DELETE', headers=headers_map, response_format=ResponseFormat.NO_CONTENT, auth=self.auth, network_session=self.network_session))
        return None