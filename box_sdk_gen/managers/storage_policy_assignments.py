...
class CreateStoragePolicyAssignmentStoragePolicy(BaseObject):
    ...
    def __init__(self, id: str, *, type: CreateStoragePolicyAssignmentStoragePolicyTypeField = CreateStoragePolicyAssignmentStoragePolicyTypeField.STORAGE_POLICY, **kwargs):
        """
...
class CreateStoragePolicyAssignmentAssignedTo(BaseObject):
    ...
    def __init__(self, type: CreateStoragePolicyAssignmentAssignedToTypeField, id: str, **kwargs):
        """
...
class UpdateStoragePolicyAssignmentByIdStoragePolicy(BaseObject):
    _discriminator = 'type', {'storage_policy'}
    def __init__(self, id: str, *, type: UpdateStoragePolicyAssignmentByIdStoragePolicyTypeField = UpdateStoragePolicyAssignmentByIdStoragePolicyTypeField.STORAGE_POLICY, **kwargs):
        """
        :param id: The ID of the storage policy to assign.
        :type id: str
...
class StoragePolicyAssignmentsManager:
    ...
    def get_storage_policy_assignments(self, resolved_for_type: GetStoragePolicyAssignmentsResolvedForType, resolved_for_id: str, *, marker: Optional[str] = None, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> StoragePolicyAssignments:
        """
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/storage_policy_assignments']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, StoragePolicyAssignments)
    def create_storage_policy_assignment(self, storage_policy: CreateStoragePolicyAssignmentStoragePolicy, assigned_to: CreateStoragePolicyAssignmentAssignedTo, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> StoragePolicyAssignment:
        """
...
policy to.
        ...
        if extra_headers is None:
            extra_headers = {}
        request_body: Dict = {'storage_policy': storage_policy, 'assigned_to': assigned_to}
        headers_map: Dict[str, str] = prepare_params({**extra_headers})
...
Example: "932483"
        ...
        return deserialize(response.data, StoragePolicyAssignment)
    def update_storage_policy_assignment_by_id(self, storage_policy_assignment_id: str, storage_policy: UpdateStoragePolicyAssignmentByIdStoragePolicy, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> StoragePolicyAssignment:
        """
