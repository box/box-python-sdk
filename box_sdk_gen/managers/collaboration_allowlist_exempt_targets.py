...
from box_sdk_gen.schemas.collaboration_allowlist_exempt_targets import CollaborationAllowlistExemptTargets

from box_sdk_gen.schemas.client_error import ClientError

from box_sdk_gen.schemas.collaboration_allowlist_exempt_target import CollaborationAllowlistExemptTarget
...
This requires `usemarker` to be set to `true`., defaults to None
        ...
        response: FetchResponse = self.network_session.network_client.fetch(FetchOptions(url=''.join([self.network_session.base_urls.base_url, '/2.0/collaboration_whitelist_exempt_targets']), method='GET', params=query_params_map, headers=headers_map, response_format=ResponseFormat.JSON, auth=self.auth, network_session=self.network_session))
        return deserialize(response.data, CollaborationAllowlistExemptTargets)
    def create_collaboration_whitelist_exempt_target(self, user: CreateCollaborationWhitelistExemptTargetUser, *, extra_headers: Optional[Dict[str, Optional[str]]] = None) -> CollaborationAllowlistExemptTarget:
        """
